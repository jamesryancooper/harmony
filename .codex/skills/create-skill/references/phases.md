...
- Create `.octon/framework/capabilities/runtime/skills/<group>/<skill_name>/`.
...
## Phase 5: Validate And Publish Host Projections

- Run `.octon/framework/capabilities/runtime/skills/_ops/scripts/validate-skills.sh <skill_name>`.
- Ensure the registry entry declares `host_adapters` and `routing` metadata.
- If validation passes, run `.octon/framework/capabilities/_ops/scripts/publish-capability-routing.sh`.
- Then run `.octon/framework/capabilities/_ops/scripts/publish-host-projections.sh` to refresh projected host surfaces when needed.

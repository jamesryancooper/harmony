 ...
 */
const runsCommand: CliCommand<GuardKitCliOptions & { runsSubcommand?: string }> = {
  name: "runs",
  description: "Query and manage run records for GuardKit",
  args: [
  ...
  async handler(args, options) {
    ...
    // Parse additional options using the shared parser
    ...
    switch (subcommand) {
      ...
      case "show": {
        if (!subArg) {
          return failure("Run ID required. Usage: guardkit runs show <runId>", 1);
        }
...
const cliConfig: KitCliConfig = {
  ...
  commands: [checkCommand, sanitizeCommand, quickCheckCommand, runsCommand],
  globalOptions: [
    { name: "file", alias: "F", description: "Read content from file", type: "string" },

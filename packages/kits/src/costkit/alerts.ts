 ...
 *
 * Monitors cost patterns and generates alerts when thresholds are crossed
 * or unusual spending patterns are detected.
 */
...
/**
 * Recent alert hashes for deduplication.
 */
 ...
 */
function createAlertHash(type: AlertType, key: string): string {
  return `${type}:${key}`;
}
...
function shouldDedupe(
  ...
  const hash = createAlertHash(type, key);
  const lastTime = recentAlertHashes.get(hash);
...
function createAlert(params: {
  ...
  if (
    params.dedupeKey &&
    shouldDedupe(params.type, params.dedupeKey, params.dedupeWindowMs)
  ) {
    return null;
  }
 ...
 */
export function checkUnusualSpendAlert(
 ...
 */
export function checkDeprecatedModelAlert(
  model: string,
  dedupeWindowMs?: number
): CostAlert | null {
 ...
 */
export function checkEstimateExceededAlert(
 ...
 * Get all alerts.
 */
export function getAlerts(): CostAlert[] {
  return [...alerts];
}
 ...
 */
export function getUnacknowledgedAlerts(): CostAlert[] {
 ...
 */
export function getAlertsBySeverity(severity: AlertSeverity): CostAlert[] {
 ...
 * Get alerts by type.
 */
export function getAlertsByType(type: AlertType): CostAlert[] {
 ...
 */
export function getRecentAlerts(withinMs: number): CostAlert[] {
 ...
 */
export function acknowledgeAlertsByType(
  type: AlertType,
  acknowledgedBy: string
): number {
  let count = 0;
 ...
 */
export function clearOldAlerts(olderThanDays: number = 30): number {
  const cutoff = Date.now() - olderThanDays * 24 * 60 * 60 * 1000;
  const before = alerts.length;
  alerts = alerts.filter((a) => new Date(a.createdAt).getTime() > cutoff);

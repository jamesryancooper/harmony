...
GUIDE="$REPO_ROOT/.octon/inputs/additive/extensions/octon-proposal-packet-lifecycle/context/routing-guide.md"
...
main() {
  ...
  assert_contains '`packet_path` alone resolves to `explain-proposal-packet`.' \
    "packet path read-only default is documented"
  assert_contains '`program_packet_path` alone resolves to `explain-proposal-program`.' \
    "program path read-only default is documented"
  assert_contains '`source_kind` defaults to `create-proposal-packet`.' \
  ...
  assert_contains '`create-proposal-program`.' \

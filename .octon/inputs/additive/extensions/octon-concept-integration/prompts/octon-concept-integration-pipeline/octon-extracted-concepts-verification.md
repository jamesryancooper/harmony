...
This prompt is **stage 2 of 4** in the Octon concept-integration prompt set.
...
That corrected final recommendation set is the default upstream input to `selected-concepts-integration-and-proposal-packet.md`.
...
Assume Octon may have changed since the earlier extraction run.

Do **not** assume the repository remains in the same state that the extraction output implicitly or explicitly relied on.

In particular, assume that:

- related concepts may already have been integrated,
- coverage may now exist under different names, abstractions, or placements,
- earlier gaps may already be closed,
- earlier recommendations may now be stale, redundant, or mis-mapped,
- and new repo-native constraints may have appeared since the extraction run.

Therefore:

- always evaluate the extraction output against the current checked-out repo,
- never preserve a recommendation solely because the first pass recommended it,
- and record a **Repository Drift Note** whenever the live repo materially changes the verdict.

The live current repository outranks stale assumptions and prior extraction state.

---

## Repository-Grounding Directive

You must inspect the live Octon repository before making any current-state claim, usefulness claim, mapping claim, or completeness claim.
...
14. any repo surfaces directly implicated by the extracted or newly discovered concepts

Do not infer repository reality from the earlier extraction output, stale proposal packets, prior audits, or this prompt alone.

If the live repo diverges from the assumptions below or from the earlier extraction output, record a **Repository Drift Note** and proceed from the observed repository state.
...
## Octon-Specific Verification Assumptions

When evaluating extracted concepts, assume the following are binding unless live repository inspection proves divergence.

### 1. Super-root and class-root model

Octon uses a single `/.octon/` super-root.

The canonical class roots are:

- `framework/` -> portable authored Octon core
- `instance/` -> repo-specific durable authored authority
- `inputs/` -> raw additive and exploratory inputs only
- `state/` -> operational truth and retained evidence
- `generated/` -> derived, rebuildable, non-authoritative outputs

`state/**` is class-organized into:

- `state/control/**`
- `state/evidence/**`
- `state/continuity/**`

Derived publication and read-model families include:

- `generated/effective/**`
- `generated/cognition/**`
- `generated/proposals/**`

### 2. Authority and precedence constraints
...
- `inputs/**` must never become a direct runtime or policy dependency.
- Conversation history, UI state, labels, comments, checks, and generated summaries are advisory unless materialized into canonical authority or evidence surfaces.

### 3. Execution, autonomy, and disclosure model

Octon's live model is run-contract-centered, fail-closed, and evidence-backed.

That means:

- the workspace charter pair is the active repo-wide objective layer beneath the constitutional kernel and repo governance declarations
- mission authority remains the continuity and long-horizon autonomy surface, not the atomic consequential execution unit
- consequential execution centers on bound run contracts and run-lifecycle control under `state/control/execution/**`
- approvals, exceptions, revocations, and other mutable execution control truth must materialize into canonical control surfaces
- retained run evidence, control-plane mutation evidence, lab evidence, publication receipts, and disclosure artifacts belong under `state/evidence/**`
- RunCard, HarnessCard, and other disclosure outputs are downstream of durable authority and retained evidence, not substitutes for them
- side effects still cross the engine-owned authorization boundary
- silence is not consent

### 4. Support-universe and adapter model

- Live support claims are bounded by `instance/governance/support-targets.yml` plus `instance/governance/exclusions/action-classes.yml`.
- Concepts that widen support claims are governance-impacting and must carry explicit support-target, admission, dossier, validation, and disclosure implications.
- Host adapters and model adapters are replaceable, non-authoritative boundaries.
- Host signals may mirror status, but they never mint authority.

### 5. Placement and overlay constraints

- Repo-specific overlay authority is legal only where the framework declares an overlay point and the instance manifest enables it.
- Proposal packets are exploratory lineage under `inputs/exploratory/proposals/**`; they are not durable authority and must point promotion targets at surfaces outside the proposal tree.
- Derived outputs are legal only after the authoritative, control, evidence, and continuity surfaces they depend on already exist elsewhere.

### 6. Non-negotiable anti-patterns

Reject or heavily down-rank any concept or mapping that would:

- create a second authoritative control plane
- make chat, session, or app memory canonical
- bypass the engine-owned execution authorization boundary
- make generated views authoritative
- turn raw `inputs/**` into runtime or policy truth
- widen support claims without explicit support-target, evidence, and disclosure treatment
- let host or model adapters mint authority
- reintroduce descendant-local `.octon/` roots or ancestor-chain locality models
- collapse run-contract, stage-attempt, evidence, or disclosure gates into an ungoverned shortcut
- smuggle policy through practices or convenience docs
- introduce shadow governance, shadow memory, shadow orchestration, or shadow approvals
- treat proposal packets, generated summaries, or exploratory notes as if they were implemented capability

...
This prompt is **stage 3 of 4** in the Octon concept-integration prompt set.
...
Its output is the default upstream execution basis for `proposal-packet-implementation-and-closeout.md`.
...
You must inspect the live repository before making any coverage claim or integration-path claim.

Start by reading the current Octon authority and topology anchors in this order unless the user provides a more specific starting point:
...
11. relevant current repo-native authority surfaces under `/.octon/instance/**`
12. relevant operational surfaces under `/.octon/state/**`
13. relevant generated publication surfaces under `/.octon/generated/**`
14. current proposal packet conventions under `/.octon/inputs/exploratory/proposals/**`

Do not stop at the README. Do not infer repository reality from marketing language or old assumptions. Do not assume absence, incompleteness, or novelty merely because the current research set presents the concept as new. If the live repo diverges from the assumptions below, record a **Repository Drift Note** and proceed from the observed repo, not the stale assumption.

---

## CURRENT OCTON REPO ASSUMPTIONS YOU SHOULD VERIFY

Assume the following are binding unless current repo inspection proves divergence:

### 1. Super-root and class-root model

Octon uses a single `/.octon/` super-root. The canonical top-level class roots are:
...
### 3. Constitutional kernel

The supreme repo-local control regime lives under:

- `/.octon/framework/constitution/**`

The umbrella architecture contract lives at:

- `/.octon/framework/cognition/_meta/architecture/specification.md`

The projected ingress surface is:

- `/.octon/AGENTS.md`

The canonical internal ingress lives at:

- `/.octon/instance/ingress/AGENTS.md`

Repo-root `AGENTS.md` and `CLAUDE.md` are thin adapters only.

### 4. State-class organization
...
### 5. Generated publication model

Treat the following as derived-only publication/read-model surfaces:
...
Repo-specific overlay authority is legal only where:

- the point is declared in `/.octon/framework/overlay-points/registry.yml`, and
- the point is enabled in `/.octon/instance/manifest.yml#enabled_overlay_points`.

When mapping repo-specific policy/contract/runtime changes, first test whether the change fits an enabled overlay point before inventing a new surface.

### 7. Proposal packet model

Proposal packets are non-canonical exploratory artifacts and must not be treated as authority. Manifest-governed proposals live under:

- active: `/.octon/inputs/exploratory/proposals/<kind>/<proposal_id>/`
- archived: `/.octon/inputs/exploratory/proposals/.archive/<kind>/<proposal_id>/`

Packets must point promotion targets at durable authority surfaces outside the proposal tree.

### 8. Mission/autonomy model

Respect Mission-Scoped Reversible Autonomy:

- approvals, exceptions, revocations, run contracts, and mutable execution control truth must materialize into canonical control surfaces,
- UI/chat/comments/labels/checks are non-authoritative projections only,
- no second mission control plane is allowed,
- and material execution must still cross the engine-owned authorization boundary before side effects occur.

---

## NON-NEGOTIABLE ANTI-PATTERNS

You must reject or heavily down-rank any concept or implementation path that would:

- create a second authoritative control plane,
- make chat/session/app memory canonical,
- bypass the engine-owned authorization boundary,
- make generated views authoritative,
- turn raw `inputs/**` into runtime or policy truth,
- reintroduce descendant-local `.octon/` roots,
- collapse governed stage/promote/finalize transitions without reversible logic,
- smuggle policy through convenience docs or agent habits,
- create shadow governance, shadow memory, shadow orchestration, or shadow approvals,
- promote proposal packets into canonical truth,
- place repo-specific authority into closed or non-overlay framework runtime domains,
- produce a docs-only or proposal-only substitute for a needed runtime capability,
- or create a thin surface addition that implies a capability exists without the control/evidence/validation wiring needed to make it real.

If a concept appears useful only by violating these rules, reject it or reformulate it into a legal Octon-native implementation path.
...
`proposal.yml` must include at least:
...
`architecture-proposal.yml` must include at least:
...
- `blocking_concepts`, `blocking_gaps`, or `blocking_dependencies`
- `implementation_motion`
- `validation_motion`

`PACKET_MANIFEST.md` must enumerate all generated packet files. `SHA256SUMS.txt` must include checksums for every packet file if your environment supports checksum generation.

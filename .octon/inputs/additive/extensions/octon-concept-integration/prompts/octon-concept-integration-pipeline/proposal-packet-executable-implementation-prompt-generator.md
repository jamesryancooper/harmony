...
4. and the baseline execution model defined in `/.octon/inputs/additive/extensions/octon-concept-integration/prompts/octon-concept-integration-pipeline/proposal-packet-implementation-and-closeout.md`.
...
This prompt is a **prompt-generation companion** to the Octon concept-integration prompt set.
...
- uses `/.octon/inputs/additive/extensions/octon-concept-integration/prompts/octon-concept-integration-pipeline/proposal-packet-implementation-and-closeout.md` as the structural and behavioral baseline,
...
Treat `/.octon/inputs/additive/extensions/octon-concept-integration/prompts/octon-concept-integration-pipeline/proposal-packet-implementation-and-closeout.md` as the baseline execution model that the generated prompt should inherit and specialize.
...
- **Proposal packet**
  Prefer the capability-managed packet artifact from the current run
  checkpoint, typically something like
  `/.octon/state/control/skills/checkpoints/octon-concept-integration/<run-id>/artifacts/proposal-packet-path.txt`,
  and then the materialized packet directory under
  `/.octon/inputs/exploratory/proposals/architecture/<proposal_id>/`. Only
  fall back to an explicitly provided packet path when those capability-managed
  artifacts are unavailable or the user explicitly overrides them.
- **Baseline execution prompt**
  Use `/.octon/inputs/additive/extensions/octon-concept-integration/prompts/octon-concept-integration-pipeline/proposal-packet-implementation-and-closeout.md`.
- **Concept-verification output**
  Prefer a capability-managed artifact from the current run checkpoint, or the proposal packet support artifact when the generator is run after packetization. Only fall back to inline `<concept_verification_output>...</concept_verification_output>` content when those artifacts are unavailable.
- **Concept-extraction output**
  Prefer a capability-managed artifact from the current run checkpoint, or the proposal packet support artifact when the generator is run after packetization. Only fall back to inline `<concept_extraction_output>...</concept_extraction_output>` content when those artifacts are unavailable.
- **Execution scope or additional user instructions**
  Usually provided directly in the thread or in tagged blocks such as `<selected_concepts>...</selected_concepts>`, `<implementation_scope>...</implementation_scope>`, `<user_notes>...</user_notes>`, `<implementation_constraints>...</implementation_constraints>`, or `<approvals>...</approvals>`.
...
- If the proposal packet is missing, first check for the capability-managed
  packet-path artifact and then the materialized proposal directory under
  `/.octon/inputs/exploratory/proposals/**`. Do **not** fabricate one from
  memory. State plainly that the proposal packet artifact is required for
  customization and request it only if those managed artifacts are genuinely
  unavailable.
...
You must inspect the live Octon repository before generating current-state instructions inside the customized prompt.
...
14. the proposal packet’s promotion targets, implementation artifacts, and closure criteria
15. any repo surfaces directly implicated by the packet’s in-scope execution path

Do not generate packet-specific execution instructions from the proposal packet alone. The present checked-out repository outranks stale packet assumptions.
...
Inspect `/.octon/inputs/additive/extensions/octon-concept-integration/prompts/octon-concept-integration-pipeline/proposal-packet-implementation-and-closeout.md` and extract at least:

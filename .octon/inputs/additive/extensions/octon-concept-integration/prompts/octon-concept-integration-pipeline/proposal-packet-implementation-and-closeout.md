...
This prompt is **stage 4 of 4** in the Octon concept-integration prompt set.

Prefer the output of `selected-concepts-integration-and-proposal-packet.md` as the default upstream execution basis.
...
- **Proposal packet** Prefer the capability-managed packet artifact from the
  current run checkpoint, typically something like
...
- **Concept-verification output** Prefer the proposal packet support artifact or a capability-managed checkpoint artifact rather than relying on thread-local context.
- **Concept-extraction output** Prefer the proposal packet support artifact or a capability-managed checkpoint artifact rather than relying on thread-local context.
- **Selected concept subset or execution scope** Usually provided directly in the prompt thread or in tagged blocks such as `<selected_concepts>...</selected_concepts>` or `<implementation_scope>...</implementation_scope>`.
- **Source artifact** Usually provided as a URL or in tagged blocks such as `<source_artifact>...</source_artifact>`.
...
- If the proposal packet is missing, first check for the capability-managed
  ...
  memory. State that the packet is missing and request it plainly only if those
...
You must inspect the live repository before making any implementation, validation, or closeout claim.
...
14. the proposal packet’s stated promotion targets, closure criteria, and implementation artifacts
15. any repo surfaces directly implicated by the packet’s in-scope changes

Do not infer implementation reality from the packet alone. The present checked-out repository outranks stale packet assumptions.

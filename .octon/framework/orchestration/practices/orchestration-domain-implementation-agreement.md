...
design material into standalone live authority under `/.octon/framework/orchestration/`.
...
1. Promote authority before merge.
   - Live orchestration artifacts must carry the contract, schema, fixture,
     policy, or operator guidance they require inside `/.octon/` before they
     are merged.
...
1. Every orchestration-domain implementation PR must cite:
   - at least one backlog ID from
     `/.octon/inputs/exploratory/plans/2026-03-10-orchestration-domain-phase0-backlog.md`
   ...
   - if the change originated from temporary design material, record that
     ...
     canonical `/.octon/framework/orchestration/` files

---
title: Copy Template
description: Copy skill template to new skill directory with spec-compliant structure.
---

# Step 2: Copy Template

> **Deprecated workflow step:** Use the `create-skill` skill for current paths and file naming.

## Input

- Validated `skill-name` from Step 1

## Actions

```text
1. Create directory: .octon/framework/capabilities/runtime/skills/<group>/<skill-name>/

2. Copy core file:
   - .octon/framework/capabilities/runtime/skills/_scaffold/template/SKILL.md -> .octon/framework/capabilities/runtime/skills/<group>/<skill-name>/SKILL.md

3. Create and copy reference files:
   - .octon/framework/capabilities/runtime/skills/_scaffold/template/references/phases.md -> .octon/framework/capabilities/runtime/skills/<group>/<skill-name>/references/phases.md
   - .octon/framework/capabilities/runtime/skills/_scaffold/template/references/io-contract.md -> .octon/framework/capabilities/runtime/skills/<group>/<skill-name>/references/io-contract.md
   - .octon/framework/capabilities/runtime/skills/_scaffold/template/references/safety.md -> .octon/framework/capabilities/runtime/skills/<group>/<skill-name>/references/safety.md
   - .octon/framework/capabilities/runtime/skills/_scaffold/template/references/examples.md -> .octon/framework/capabilities/runtime/skills/<group>/<skill-name>/references/examples.md
   - .octon/framework/capabilities/runtime/skills/_scaffold/template/references/validation.md -> .octon/framework/capabilities/runtime/skills/<group>/<skill-name>/references/validation.md

4. Create empty directories:
   - .octon/framework/capabilities/runtime/skills/<group>/<skill-name>/scripts/
   - .octon/framework/capabilities/runtime/skills/<group>/<skill-name>/assets/

5. Do not edit host-facing skill directories directly.
   Host projections are regenerated later from the published routing view.
```

## Directory Structure

Creates agentskills.io spec-compliant structure:

```text
.octon/framework/capabilities/runtime/skills/<group>/<skill-name>/
├── SKILL.md              # Core skill definition
├── references/           # Progressive disclosure content
│   ├── phases.md         # Detailed phase behavior
│   ├── io-contract.md    # Inputs/outputs/dependencies + CLI usage
│   ├── safety.md         # Safety policies
│   ├── examples.md       # Full examples
│   └── validation.md     # Acceptance criteria
├── scripts/              # Executable code (empty)
└── assets/               # Static resources (empty)
```

## Verification

- Directory `.octon/framework/capabilities/runtime/skills/<group>/<skill-name>/` exists
- File `.octon/framework/capabilities/runtime/skills/<group>/<skill-name>/SKILL.md` exists
- All reference files exist in `.octon/framework/capabilities/runtime/skills/<group>/<skill-name>/references/`
- Empty directories created (`scripts/`, `assets/`)
- Host projection regeneration is deferred until routing publication

## Idempotency

**Check:** Is template already copied?
- [ ] Directory `.octon/framework/capabilities/runtime/skills/<group>/<skill-name>/` exists
- [ ] File `.octon/framework/capabilities/runtime/skills/<group>/<skill-name>/SKILL.md` exists
- [ ] All reference files exist
- [ ] Host projection regeneration is handled in the post-registration phase

**If Already Complete:**
- Verify directory structure is complete
- Skip to next step if all files present

**Marker:** `checkpoints/create-skill/<skill-name>/02-copy.complete`

## Output

- New skill directory with template files
- Spec-compliant structure with progressive disclosure
- Proceed to Step 3

---
name: verify
title: Verify Policy Proposal Audit Completion
description: Validate mode coverage, report presence, and package-delta receipts.
---

# Step 12: Verify Policy Proposal Audit Completion

## Conditional Standard-Governed Package Gate

This step must run the fail-closed policy proposal validator stack:

- `bash .octon/framework/assurance/runtime/_ops/scripts/validate-proposal-standard.sh --package "<target-package>"`
- `bash .octon/framework/assurance/runtime/_ops/scripts/validate-policy-proposal.sh --package "<target-package>"`
...
2. Run the baseline, policy, and implementation-readiness validators and record the result in
   `validation.md`.

---
name: verify
title: Verify Migration Proposal Audit Completion
description: Validate mode coverage, report presence, and package-delta receipts.
---

# Step 12: Verify Migration Proposal Audit Completion

## Conditional Standard-Governed Package Gate

This step must run the fail-closed migration proposal validator stack:

- `bash .octon/framework/assurance/runtime/_ops/scripts/validate-proposal-standard.sh --package "<target-package>"`
- `bash .octon/framework/assurance/runtime/_ops/scripts/validate-migration-proposal.sh --package "<target-package>"`
...
- [ ] Migration proposal validator passes
...
2. Run the baseline, migration, and implementation-readiness validators and record the result in
   `validation.md`.

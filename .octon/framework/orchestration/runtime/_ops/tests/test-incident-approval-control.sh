...
VERIFY_SCRIPT=".octon/framework/orchestration/runtime/_ops/scripts/verify-approval-artifact.sh"
INCIDENT_SCRIPT=".octon/framework/orchestration/runtime/_ops/scripts/manage-incident.sh"
...
create_fixture() {
  ...
  mkdir -p "$fixture_root/.octon/framework/orchestration/governance"
  mkdir -p "$fixture_root/.octon/state/evidence/decisions/repo/approvals"
  ...
  cp "$REPO_ROOT/.octon/framework/orchestration/governance/incidents.md" "$fixture_root/.octon/framework/orchestration/governance/incidents.md"
  cp "$REPO_ROOT/.octon/framework/orchestration/governance/approver-authority-registry.json" "$fixture_root/.octon/framework/orchestration/governance/approver-authority-registry.json"
  ...
  cp "$REPO_ROOT/.octon/state/evidence/decisions/repo/approvals/README.md" "$fixture_root/.octon/state/evidence/decisions/repo/approvals/README.md"
...
case_valid_close_requires_approval() {
  ...
  [[ -f "$fixture_root/.octon/framework/orchestration/runtime/incidents/inc-test-001/closure.md" ]]
  yq -r '.status' "$fixture_root/.octon/framework/orchestration/runtime/incidents/inc-test-001/incident.yml" | grep -qx "closed"
}

# Engagement / Project Profile / Change Package Compiler v1
...
a Change Package, Decision Requests, a context-pack request, and a first
...
Project Profile, per-engagement Objective Brief, Change Package, Decision Request,
...
- `.octon/state/control/engagements/<engagement-id>/change-package.yml`
...
Change Package may cite connector posture to stage, block, deny, or request a
...
- `.octon/generated/cognition/projections/materialized/change-packages/<change-package-id>.yml`
...
6. `decide_mode` returns run-only, mission-required, stage-only, blocked, or
   denied.
7. `compile_change_package` assembles plan, risk, validation, rollback, support,

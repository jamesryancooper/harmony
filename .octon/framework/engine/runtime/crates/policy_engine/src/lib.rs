...
fn validate_declared_skill_services(
    ...
    let Ok(manifest) = load_yaml::<ServicesManifest>(&services_manifest_path) else {
        return Some(deny(
            ...
            None,
            None,
            "Ensure .octon/framework/capabilities/runtime/services/manifest.yml exists and is valid",
            None,
        ));
    };
...
mod tests {
    ...
    fn split_allowed_tools_respects_parentheses_depth() {
        let raw =
            "Read Bash(git diff --name-only) Write(.octon/generated/**) Bash(rg deny-by-default)";

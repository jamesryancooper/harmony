# Monorepo Architecture - Polyglot (TS + Python) – Octon Methodology

Below is a concrete, Octon‑aligned blueprint for a **polyglot (TypeScript + Python) modular‑monolith monorepo** using **Turborepo** (task runner & cache), **pnpm** (TS/Node workspaces), and **uv** (Python workspaces). It maps vertical slices + hexagonal boundaries to Octon’s *kits/control‑plane vs. runtimes* model, and wires TypeScript + Python into a single deterministic task graph.
...
Octon specifics—**kits as control plane** (`packages/kits/*`), a **platform flow runtime service** whose LangGraph-based implementation lives under `platform/runtimes/flow-runtime/**`, and strict module boundaries—come straight from the Octon blueprint and AI‑Toolkit docs. See also [`runtime-architecture.md`](./runtime-architecture.md) for the canonical runtime model.  
...
│  └─ kits/                         # Octon control-plane kits (TS)
...
Octon requires **published interfaces only** with consumer‑driven contracts and CI gates—exactly what this does.
...
## 4) Slices + Hexagonal boundaries → Octon mapping
...
This clean split supports Octon’s “speed with safety, determinism, and guided autonomy.”
...
6. **Add Octon kits** under `packages/kits/*` (e.g., FlowKit, AgentKit) as TS libraries; they export contracts only (no runtime).
...
```bash
pnpm --filter @octon/contracts run gen   # TS types + Python client
```
...
| name                          | language | type    | role (Octon)                   | slice/layer       |
| ----------------------------- | -------- | ------- | -------------------------------- | ----------------- |
...
Octon alignment: *monolith‑first, hexagonal boundaries, control plane in kits, one shared runtime, contract‑first gates, OTel everywhere*.  
...
## 15) Validation checklist (Octon‑aligned)
...
- [ ] Observability hooks present (OTel spans/ids in changed flows) per Octon practice.
...
This design follows the Octon repository blueprint (slices, kits, control plane, platform runtime service) and its agent/flow responsibilities—so the **control plane stays in TypeScript**, **flows/agents execute via the shared platform flow runtime service**, and **contracts** keep everything deterministic and testable.

...
|----------|------|-----------------|
| Activity log | `.octon/state/continuity/repo/log.md` | Yes (append-first markdown contract) |
...
| `.octon/instance/cognition/decisions/` | Knowledge Plane | **Source of truth** for durable decision records | Immutable-by-record (supersede via new ADR) |
...
2. **Keep continuity logging append-first** (`.octon/state/continuity/repo/log.md`) and store per-session run evidence under `.octon/state/evidence/runs/` when needed.

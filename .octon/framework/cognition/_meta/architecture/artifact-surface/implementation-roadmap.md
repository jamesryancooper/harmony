...
- Zod schema registry + `octon-content validate`.
- Basic reference parsing + resolution (build a refs list).
- Generate `.octon/content/content.json` (unified docs array).
...
- Generate SQLite index `.octon/content/content.sqlite`.
- Implement `refs` table and `graph.json`.
- Add FTS5 for prose search.
- Implement `octon-content query` and a small set of canned queries (`impacted-by`, `missing-seo`, `untranslated`).
...
- Implement restricted `octon-include` directive for snippets only.
...
- Implement lease system in `.octon/leases` per spec.
- Implement "write set" detection (files changed → doc keys).
- Add optional CI overlap check for hot sets (configurable).
- Add lifecycle checks for `/.octon/state/continuity/repo/log.md` append-first behavior and `/.octon/state/evidence/runs/` retention contract conformance.
...
- `octon-content runtime:check` CLI command (drift detection).
- `octon-content runtime:sync` CLI command (PR generation).

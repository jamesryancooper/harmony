...
|---|---|---|
| Execution Kernel Plane | How is executable work dispatched? | `.octon/framework/engine/runtime/` |
| Service Plane | What typed runtime operations exist? | `.octon/framework/capabilities/runtime/services/` |
| Ingress Plane | How does human intent enter the system? | `.octon/framework/capabilities/runtime/commands/` |
| Capability Plane | What atomic execution units are available? | `.octon/framework/capabilities/runtime/skills/` |
| Orchestration Plane | How are multi-step missions sequenced? | `.octon/framework/orchestration/runtime/workflows/` |
| Assurance Plane | What must pass before completion? | `.octon/framework/assurance/{runtime,practices}/` |
| Continuity Plane | What happened, why did it happen, and what is next? | `.octon/state/continuity/repo/{log.md,tasks.json,entities.json,next.md} + .octon/state/evidence/{decisions/repo/,runs/}` |
| Knowledge Plane | What durable system context and decisions exist? | `.octon/framework/cognition/runtime/{context,decisions,evidence,evaluations,projections,knowledge-plane}/` |
| Artifact Plane | What durable deliverables/evidence are produced? | `.octon/generated/` (with optional artifact compilation architecture under `/.octon/framework/cognition/_meta/architecture/artifact-surface/`) |
...
`/.octon/framework/cognition/_meta/architecture/artifact-surface/` defines the artifact compiler architecture used by the Artifact Plane. Runtime overlays in that surface are optional implementation layers; Artifact Plane ownership remains foundational.
...
- `.octon/framework/cognition/_meta/architecture/artifact-surface/README.md`

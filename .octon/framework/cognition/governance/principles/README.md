---
title: Octon Principles
description: Canonical principles index and thresholds that translate Octon pillars into enforceable day-to-day engineering and governance behavior.
---
...
Octon principles are the decision layer between pillars and methodology. They define how day-to-day technical choices preserve the six pillars while optimizing for velocity, maintainability, scalability, reliability, security, and minimal sufficient complexity.
...
- When arbitration is applied, write an append-only decision record (for example ADR under `.octon/instance/cognition/decisions/`).
- Include arbitration rationale and affected rule IDs in run receipts/digests (for example `.octon/state/evidence/runs/*/receipt.json` note fields).
...
- `.octon/framework/cognition/governance/principles/charter-map.yml`
- `.octon/framework/cognition/governance/principles/arbitration-and-precedence.md`
- `.octon/framework/cognition/governance/controls/convivial-impact-minimums.md`
...
- `.octon/framework/cognition/_meta/architecture/observability-requirements.md`

...
usage() {
  cat <<'EOF'
Usage: validate-engagement-change-package-compiler.sh [--root <repo-root>] [--change-package <path>] [--cli-help <path>]

Validates Engagement / Project Profile / Change Package Compiler governance gates.
Relative --change-package and --cli-help paths resolve from --root.
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --change-package)
      if [[ $# -lt 2 || "${2:-}" == --* ]]; then
        echo "[ERROR] --change-package requires a path" >&2
        usage >&2
        exit 2
      fi
      CHANGE_PACKAGE_ARG="$2"
      shift 2
      ;;
    --cli-help)
      if [[ $# -lt 2 || "${2:-}" == --* ]]; then
...
COMPILER_POLICY="$OCTON_DIR/instance/governance/policies/engagement-change-package-compiler.yml"
...
check_static_policy_surfaces() {
  echo "== Engagement Change Package Compiler Policy Validation =="
  ...
  require_yaml_file "$COMPILER_POLICY" "engagement-change-package-compiler-governance-v1"
  ...
  require_json_schema_file "$OCTON_DIR/framework/engine/runtime/spec/change-package-v1.schema.json" "Change Package runtime schema"
  ...
  require_json_schema_file "$OCTON_DIR/framework/constitution/contracts/runtime/change-package-v1.schema.json" "Change Package constitutional runtime schema"
  ...
  require_yq "$CHANGE_PACKAGE_SCHEMA" 'has("$schema") and (.required | length > 0)' "change-package schema carries JSON Schema metadata and required fields"
  require_yq "$CONTRACT_REGISTRY" '.integration_surfaces.engagement_change_package_compiler_contracts.engagement_objective_brief_schema == ".octon/framework/constitution/contracts/runtime/engagement-objective-brief-v1.schema.json"' "constitutional registry places Engagement Objective Brief under runtime contracts"
  require_yq "$ARCH_REGISTRY" '.path_families.engagement_change_package_compiler.canonical_paths[]? | select(. == ".octon/framework/constitution/contracts/runtime/engagement-objective-brief-v1.schema.json")' "architecture registry includes runtime Objective Brief contract"
  if yq -e '.path_families.engagement_change_package_compiler.canonical_paths[]? | select(. == ".octon/framework/constitution/contracts/objective/engagement-objective-brief-v1.schema.json")' "$ARCH_REGISTRY" >/dev/null 2>&1; then
...
check_compiler_gates() {
  ...
  for section in engagement_ref project_profile_ref objective_brief_schema_ref objective_control_root objective_brief_ref objective_brief_evidence_refs objective_brief_authority_boundary authority_binding support_posture capability_posture connector_posture evidence_profile context_pack decision_requests rollback validation risk_materiality placement run_contract_candidate run_contract_readiness_evidence_refs outcome; do
    require_list_value "$COMPILER_POLICY" '.change_package_required_sections' "$section" "change package required section declared: $section"
  done
...
change_package_paths() {
  if [[ -n "$CHANGE_PACKAGE_ARG" ]]; then
    resolve_root_path "$CHANGE_PACKAGE_ARG"
    return 0
  fi

  if [[ -n "${OCTON_ENGAGEMENT_CHANGE_PACKAGE:-}" ]]; then
    resolve_root_path "$OCTON_ENGAGEMENT_CHANGE_PACKAGE"
    return 0
  fi

  local engagements_root="$OCTON_DIR/state/control/engagements"
  [[ -d "$engagements_root" ]] || return 0
  find "$engagements_root" -type f -name 'change-package.yml' -print
}
...
check_change_package_support_tuple() {
  local change_package="$1"
  local tuple_id="$2"
  local claim_effect="$3"

  [[ "$claim_effect" == "admitted-live-claim" ]] \
    && pass "ready Change Package claims admitted live support" \
    || fail "ready Change Package must claim admitted-live-claim support"

  if [[ -z "$tuple_id" ]]; then
    fail "ready Change Package missing support tuple id"
    return 0
  fi

  require_yq "$SUPPORT_TARGETS" ".tuple_admissions[]? | select(.tuple_id == \"$tuple_id\" and .claim_effect == \"admitted-live-claim\")" "ready Change Package support tuple is admitted live: ${change_package#$ROOT_DIR/}"
}

check_change_package_capabilities() {
  local change_package="$1"
  ...
  while IFS= read -r pack_id; do
    ...
    [[ "$status" == "admitted" ]] \
      && pass "ready Change Package capability pack admitted: $pack_id" \
      || fail "ready Change Package capability pack is not admitted: $pack_id"
  done < <(yq -r '.capability_posture.pack_ids[]? // ""' "$change_package" 2>/dev/null || true)
}

check_change_package_connectors() {
  local change_package="$1"
  local outcome="$2"
  ...
  while IFS=$'\t' read -r connector requested_live_effect requested_use operation_mode operation support_status; do
    ...
    if [[ -z "$live_route" || "$live_route" == "null" ]]; then
      fail "Change Package references unknown connector: $connector"
      continue
    fi
    if [[ "$requested_use" == "live_effect" || "$requested_use" == "effectful" ]]; then
      [[ "$live_route" == "denied" ]] \
        && fail "Change Package requests live effectful non-admitted connector use: $connector" \
        || fail "Change Package connector live route is not fail-closed: $connector"
    elif [[ "$outcome" == "ready_for_authorization" && "$support_status" != "admitted" && "$support_status" != "supported" ]]; then
      fail "ready Change Package includes non-admitted connector: $connector"
    else
      pass "Change Package connector posture is non-live or staged: $connector"
    fi
  done < <(yq -r '((.connector_posture.connectors // []) + (.connector_posture.requested_connectors // []))[]? | [(.connector_class_id // .connector_id // ""), (.requested_live_effect // false), (.requested_use // ""), (.operation_mode // ""), (.operation // ""), (.support_status // "unadmitted")] | @tsv' "$change_package" 2>/dev/null || true)
}

check_ready_change_package_placement() {
  local change_package="$1"
  local family

  require_yq "$change_package" '.placement.path_family_registry_ref == ".octon/instance/governance/engagements/path-families.yml"' "ready Change Package binds engagement path-family registry"
  while IFS= read -r family; do
    [[ -n "$family" ]] || continue
    require_yq "$change_package" ".placement.runtime_write_family_refs[]? | select(. == \"$family\")" "ready Change Package declares runtime-write family: $family"
  done < <(yq -r '.runtime_write_requires_registered_families[]?' "$PATH_FAMILIES")
}
...
check_authority_binding_sources() {
  local change_package="$1"
  if yq -r '.authority_binding | .. | select(tag == "!!str")' "$change_package" 2>/dev/null | grep -E '^\.octon/generated/' >/dev/null 2>&1; then
...
check_objective_brief_placement() {
  ...
  objective_ref="$(yq -r '.objective_brief_ref // ""' "$change_package")"
  ...
  require_yq "$change_package" '.objective_brief_authority_boundary.objective_brief_is_workspace_charter_authority == false and .objective_brief_authority_boundary.material_execution_authorized_by_objective_brief == false' "Change Package preserves Objective Brief non-authority boundary"
  require_yq "$change_package" '(.objective_brief_evidence_refs | length) > 0 and (.objective_brief_evidence_refs[]? | test("^\\.octon/state/evidence/engagements/[^/]+/objective/"))' "Change Package binds retained Objective Brief evidence"
...
check_run_contract_readiness_evidence() {
  local change_package="$1"
  require_yq "$change_package" '(.run_contract_readiness_evidence_refs | length) > 0 and (.run_contract_readiness_evidence_refs[]? | test("^\\.octon/state/evidence/engagements/[^/]+/run-contract-readiness/"))' "Change Package binds run-contract readiness evidence"
}

check_run_contract_candidate() {
  local change_package="$1"
  local candidate_ref candidate_path
  candidate_ref="$(yq -r '.run_contract_candidate.ref // .run_contract_candidate.candidate_ref // .run_contract_candidate.path // ""' "$change_package")"
...
check_decision_request_state() {
  ...
  while IFS= read -r decision_ref; do
    ...
    if [[ "$status" == "open" ]]; then
      fail "ready Change Package must not have unresolved Decision Request: $decision_ref"
    else
      pass "Decision Request is not open: $decision_ref"
    fi
  done < <(yq -r '.decision_requests[]? | .decision_ref // .decision_request_ref // .ref // .path // .' "$change_package" 2>/dev/null || true)
}
...
check_non_ready_reasoning() {
  ...
  case "$outcome" in
    stage_only|blocked|denied|requires_decision|escalate)
      if yq -e '((.outcome.blockers // []) | length > 0) or ((.outcome.reason_refs // []) | length > 0) or ((.blockers // []) | length > 0) or ((.reason_refs // []) | length > 0)' "$change_package" >/dev/null 2>&1; then
        pass "non-ready Change Package carries blocker or reason refs"
      else
        fail "non-ready Change Package must carry blocker or reason refs"
      fi
      ;;
  esac
}
...
validate_change_package() {
  ...
  if [[ ! -f "$change_package" ]]; then
    fail "Change Package missing: $change_package"
    return 0
  fi
  yq -e '.' "$change_package" >/dev/null 2>&1 \
    && pass "Change Package parses as YAML: ${change_package#$ROOT_DIR/}" \
    || { fail "Change Package must parse as YAML: ${change_package#$ROOT_DIR/}"; return 0; }

  while IFS= read -r section; do
    [[ -n "$section" ]] || continue
    require_yq "$change_package" "has(\"$section\")" "Change Package includes required section $section: ${change_package#$ROOT_DIR/}"
  done < <(yq -r '.change_package_required_sections[]?' "$COMPILER_POLICY")

  if contains_proposal_path "$change_package"; then
    fail "Change Package must not depend on proposal-local paths: ${change_package#$ROOT_DIR/}"
  else
    pass "Change Package has no proposal-local runtime dependency: ${change_package#$ROOT_DIR/}"
  fi

  check_objective_brief_placement "$change_package"
  check_run_contract_readiness_evidence "$change_package"
  check_decision_request_shapes "$change_package"
  check_authority_binding_sources "$change_package"
  check_run_contract_candidate "$change_package"

  outcome="$(yq -r '.outcome.status // .outcome // ""' "$change_package")"
  require_yq "$COMPILER_POLICY" ".allowed_final_outcomes[]? | select(. == \"$outcome\")" "Change Package outcome is explicit: $outcome"
  check_non_ready_reasoning "$change_package" "$outcome"

  profile="$(yq -r '.evidence_profile.profile_id // .evidence_profile.selected_profile // ""' "$change_package")"
  if [[ -z "$profile" ]]; then
    fail "Change Package must select an evidence profile"
  elif is_known_profile "$profile"; then
    pass "Change Package selected known evidence profile: $profile"
  else
    fail "Change Package selected unknown evidence profile: $profile"
  fi

  if [[ "$outcome" == "requires_decision" ]]; then
    require_yq "$change_package" '(.decision_requests | length) > 0' "requires_decision Change Package emits Decision Requests"
  fi

  check_change_package_connectors "$change_package" "$outcome"
  ...
  if [[ "$outcome" == "ready_for_authorization" ]]; then
    ...
    check_ready_change_package_placement "$change_package"
    require_yq "$change_package" '(.context_pack.receipt_ref // "") != ""' "ready Change Package binds context-pack receipt"
    require_yq "$change_package" '((.rollback.plan_ref // .rollback.rollback_plan_ref // .rollback.ref // "") != "")' "ready Change Package binds rollback plan"
    require_yq "$change_package" '(.runtime_authorization.grant_bundle_ref // .authority_binding.grant_bundle_ref // "") != ""' "ready Change Package binds grant bundle"

    tuple_id="$(yq -r '.support_posture.tuple_id // .support_posture.support_target_tuple_ref // ""' "$change_package")"
    claim_effect="$(yq -r '.support_posture.claim_effect // ""' "$change_package")"
    check_change_package_support_tuple "$change_package" "$tuple_id" "$claim_effect"
    check_change_package_capabilities "$change_package"
    check_decision_request_state "$change_package"
  fi
}

check_change_packages() {
  local found=0
  local change_package

  while IFS= read -r change_package; do
    [[ -n "$change_package" ]] || continue
    found=1
    validate_change_package "$change_package"
  done < <(change_package_paths)

  if [[ "$found" -eq 0 ]]; then
    pass "no live Engagement Change Packages present; static gates are active"
  fi
}
...
validator_result_add_negative_control \
  ...
  "ready-change-package-with-non-admitted-connector-fails" \
  "ready-change-package-missing-rollback-fails"
...
validator_result_add_schema_version \
  "engagement-change-package-compiler-governance-v1" \
...
if [[ "$errors" -eq 0 ]]; then
  emit_validator_result "validate-engagement-change-package-compiler.sh" "engagement_change_package_compiler_governance" "semantic" "semantic" "pass"
else
  emit_validator_result "validate-engagement-change-package-compiler.sh" "engagement_change_package_compiler_governance" "semantic" "existence" "fail"
fi
[[ "$errors" -eq 0 ]]

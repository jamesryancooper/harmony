...
check_control_contracts() {
  ...
  require_jq "$CONTROL_CONTRACT" '.rulesets.current_live_main.expectation == "target-route-neutral"' "control contract records current live route-neutral ruleset" "control contract must record current live route-neutral ruleset"
  require_jq "$CONTROL_CONTRACT" '(.rulesets.current_live_main.required_checks | index("route_neutral_closeout_validation") != null) and (.rulesets.current_live_main.required_checks | index("branch_naming_validation") != null) and (.rulesets.current_live_main.required_checks | index("route_aware_autonomy_validation") != null) and (.rulesets.current_live_main.required_checks | index("exact_source_sha_validation") != null)' "control contract records current live route-neutral checks" "control contract must record current live route-neutral checks"
  ...
  require_jq "$CONTROL_CONTRACT" '.rulesets.target_route_neutral_main.live_mutation_performed_by_this_projection == false' "control contract says repo-local projection itself did not mutate live rulesets" "control contract must not claim repo-local projection performed live mutation"
  ...
  require_literal "$GITHUB_RUNBOOK" "## Route-Neutral Main Ruleset Change Plan" "GitHub runbook includes route-neutral ruleset change plan" "GitHub runbook must include route-neutral ruleset change plan"
  require_literal "$GITHUB_RUNBOOK" "gh api repos/<owner>/<repo>/rulesets" "GitHub runbook includes read-only ruleset list command" "GitHub runbook must include read-only ruleset list command"
  require_literal "$GITHUB_RUNBOOK" "Do not run unless a maintainer accepts a new ruleset change or rollback packet" "GitHub runbook blocks live mutation before maintainer acceptance" "GitHub runbook must block live mutation before maintainer acceptance"
  require_literal "$GITHUB_RUNBOOK" "--method PUT" "GitHub runbook proposes reviewed ruleset update command" "GitHub runbook must propose reviewed ruleset update command"

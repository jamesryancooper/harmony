...
main() {
  ...
  require_yq '.excluded_surfaces[] | select(. == "host://github-control-plane")' "$COVERAGE_LEDGER" "coverage ledger excludes github host adapter from the live claim"
  require_yq '.excluded_surfaces[] | select(. == "host://studio-control-plane")' "$COVERAGE_LEDGER" "coverage ledger excludes studio host adapter from the live claim"
  require_yq '.excluded_surfaces[] | select(. == "model://frontier-governed")' "$COVERAGE_LEDGER" "coverage ledger excludes frontier-governed model from the live claim"
  require_yq '.excluded_surfaces[] | select(. == "capability-pack://browser")' "$COVERAGE_LEDGER" "coverage ledger excludes browser from the live claim"
  require_yq '.excluded_surfaces[] | select(. == "capability-pack://api")' "$COVERAGE_LEDGER" "coverage ledger excludes api from the live claim"
  require_yq '(.excluded_surfaces | length) >= 1' "$COVERAGE_LEDGER" "coverage ledger records explicit non-live exclusions"

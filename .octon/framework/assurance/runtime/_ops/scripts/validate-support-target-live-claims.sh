...
main() {
  ...
  require_yq '(.known_limits | length) == 0' "$AUTHORED_CARD" "authored HarnessCard has no unresolved known limits"
  require_yq '(.known_limits | length) == 0' "$RELEASE_CARD" "active release HarnessCard has no unresolved known limits"

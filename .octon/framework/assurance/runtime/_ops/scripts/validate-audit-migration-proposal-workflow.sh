...
WORKFLOW_DIR="$OCTON_DIR/orchestration/runtime/workflows/audit/audit-migration-proposal"
...
pass(){ echo "[OK] $1"; }
yq -e '.name == "audit-migration-proposal"' "$WORKFLOW_DIR/workflow.yml" >/dev/null 2>&1 && pass "workflow id matches" || fail "workflow id matches"

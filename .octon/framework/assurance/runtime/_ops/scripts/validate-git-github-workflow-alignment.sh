...
main() {
  ...
  require_literal "$GITHUB_RUNBOOK" "Route-Neutral Main Ruleset Change Plan" "GitHub runbook documents route-neutral ruleset change plan" "GitHub runbook must document route-neutral ruleset change plan"
  ...
  require_literal "$GITHUB_RUNBOOK" "Do not run unless a maintainer accepts a new ruleset change or rollback packet" "GitHub runbook gates live ruleset mutation on maintainer acceptance" "GitHub runbook must gate live ruleset mutation on maintainer acceptance"
  ...
  require_literal "$MAIN_CHANGE_ROUTE_GUARD" "Accepted modes are branch-pr merged PR, direct-main Change receipt, hosted branch-no-pr provider evidence or Change receipt, or authorized break-glass." "main guard fails closed by Change route" "main guard must fail closed by Change route"

...
WORKFLOW_DIR="$OCTON_DIR/orchestration/runtime/workflows/meta/create-architecture-proposal"
...
pass(){ echo "[OK] $1"; }
yq -e '.name == "create-architecture-proposal"' "$WORKFLOW_DIR/workflow.yml" >/dev/null 2>&1 && pass "workflow id matches" || fail "workflow id matches"

...
VALIDATOR="$ROOT_DIR/.octon/framework/assurance/runtime/_ops/scripts/validate-engagement-change-package-compiler.sh"
FIXTURE_ROOT="$ROOT_DIR/.octon/framework/assurance/runtime/_ops/fixtures/engagement-change-package-compiler-v1/valid"
...
build_generated_fixture() {
  ...
  cat >"$case_root/.octon/state/control/engagements/eng-001/change-package.yml" <<'YAML'
schema_version: "change-package-v1"
change_package_id: "wp-001"
...
implementation_plan_summary: "Fixture Change Package for validator coverage."
impact_map: {}
risk_materiality:
...
scope_out:
  - ".octon/state/control/engagements/eng-001/change-package.yml"
done_when:
  - "Fixture Change Package is validated."
acceptance_criteria:
...
allowed_resolutions:
  - "approval"
subject_refs:
  change_package_ref: ".octon/state/control/engagements/eng-001/change-package.yml"
...
run_validator() {
  ...
  if bash "$VALIDATOR" \
    --root "$case_root" \
    --change-package ".octon/state/control/engagements/eng-001/change-package.yml" \
...
case_schema_metadata_missing_fails() {
  ...
  case_root="$(new_case)"
  schema="$case_root/.octon/framework/engine/runtime/spec/change-package-v1.schema.json"
...
case_objective_brief_wrong_root_fails() {
  ...
  yq -i '.objective_brief_ref = ".octon/instance/charter/workspace.md"' \
    "$case_root/.octon/state/control/engagements/eng-001/change-package.yml"
  ! run_validator "$case_root"
}

case_objective_brief_workspace_authority_fails() {
  local case_root change_package objective
  case_root="$(new_case)"
  change_package="$case_root/.octon/state/control/engagements/eng-001/change-package.yml"
  objective="$case_root/.octon/state/control/engagements/eng-001/objective/objective-brief.yml"
  yq -i '.objective_brief_authority_boundary.objective_brief_is_workspace_charter_authority = true' "$change_package"
...
case_missing_run_readiness_evidence_fails() {
  local case_root change_package
  case_root="$(new_case)"
  change_package="$case_root/.octon/state/control/engagements/eng-001/change-package.yml"
  yq -i 'del(.run_contract_readiness_evidence_refs)' "$change_package"
  ! run_validator "$case_root"
}
...
case_generated_authority_ref_fails() {
  ...
  case_root="$(new_case)"
  change_package="$case_root/.octon/state/control/engagements/eng-001/change-package.yml"
  yq -i '.authority_binding.authority_ref = ".octon/generated/cognition/projections/materialized/change-packages/wp-001.yml"' "$change_package"
  ! run_validator "$case_root"
}
...
case_ready_with_stage_only_profile_fails() {
  ...
  case_root="$(new_case)"
  change_package="$case_root/.octon/state/control/engagements/eng-001/change-package.yml"
  yq -i '.evidence_profile.selected_profile = "stage-only"' "$change_package"
  ! run_validator "$case_root"
}
...
case_non_admitted_connector_ready_fails() {
  ...
  case_root="$(new_case)"
  change_package="$case_root/.octon/state/control/engagements/eng-001/change-package.yml"
  yq -i '.outcome.status = "ready_for_authorization" | .outcome.blockers = [] | .connector_posture.requested_connectors += [{"connector_class_id": "api", "requested_live_effect": false, "operation": "inventory"}]' "$change_package"
  ! run_validator "$case_root"
}
...
case_stage_only_without_reason_fails() {
  ...
  case_root="$(new_case)"
  change_package="$case_root/.octon/state/control/engagements/eng-001/change-package.yml"
  yq -i '.outcome.status = "stage_only" | .outcome.blockers = [] | .outcome.reason_refs = []' "$change_package"
  ! run_validator "$case_root"
}

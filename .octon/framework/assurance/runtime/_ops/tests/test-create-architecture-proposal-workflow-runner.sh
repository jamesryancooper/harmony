...
run_create_workflow() {
  ...
  (
    cd "$fixture_root"
    "$RUNNER" workflow run create-architecture-proposal \
...
case_scaffold_passes() {
  ...
  output="$(run_create_workflow "$fixture_root" runtime-package "Runtime Architecture Proposal" octon-internal ".octon/framework/orchestration/runtime/example.md")"
  bundle_root="$(printf '%s\n' "$output" | sed -n 's/^bundle_root: //p' | tail -n 1)"
  proposal_root="$fixture_root/.octon/inputs/exploratory/proposals/architecture/runtime-package"
...
case_repo_local_scaffold_passes() {
  ...
  fixture_root="$(new_fixture_repo)"
  output="$(run_create_workflow "$fixture_root" repo-local-proposal "Repo Local Architecture Proposal" repo-local "CHANGELOG.md")"
  proposal_root="$fixture_root/.octon/inputs/exploratory/proposals/architecture/repo-local-proposal"
...
main() {
  case_scaffold_passes && pass "create-architecture-proposal scaffolds a architecture proposal with workflow bundle receipts" || fail "create-architecture-proposal scaffolds a architecture proposal with workflow bundle receipts"
  case_repo_local_scaffold_passes && pass "create-architecture-proposal scaffolds a repo-local proposal" || fail "create-architecture-proposal scaffolds a repo-local proposal"
  case_duplicate_id_fails && pass "create-architecture-proposal duplicate proposal failure writes receipts" || fail "create-architecture-proposal duplicate proposal failure writes receipts"
  echo

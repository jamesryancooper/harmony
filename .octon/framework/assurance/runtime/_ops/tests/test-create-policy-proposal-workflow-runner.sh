...
case_scaffold_passes() {
  ...
  output="$(run_create_workflow "$fixture_root" runtime-package "Runtime Policy Proposal" octon-internal ".octon/framework/orchestration/runtime/example.md")"
  bundle_root="$(printf '%s\n' "$output" | sed -n 's/^bundle_root: //p' | tail -n 1)"
  proposal_root="$fixture_root/.octon/inputs/exploratory/proposals/policy/runtime-package"
...
case_repo_local_scaffold_passes() {
  ...
  proposal_root="$fixture_root/.octon/inputs/exploratory/proposals/policy/repo-local-proposal"

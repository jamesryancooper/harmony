...
run_create_workflow() {
  ...
  (
    ...
    "$RUNNER" workflow run create-design-proposal \
      ...
      --set "promotion_scope=octon-internal" \
...
latest_bundle_root() {
  local fixture_root="$1"
  find "$fixture_root/.octon/state/evidence/runs/workflows" -maxdepth 1 -mindepth 1 -type d | sort | tail -n 1
}

case_domain_runtime_scaffold_passes() {
  local fixture_root output bundle_root package_root
  fixture_root="$(new_fixture_repo)"
  output="$(run_create_workflow "$fixture_root" runtime-package "Runtime Package" domain-runtime ".octon/framework/orchestration/runtime/example.md")"
  bundle_root="$(printf '%s\n' "$output" | sed -n 's/^bundle_root: //p' | tail -n 1)"
  package_root="$fixture_root/.octon/inputs/exploratory/proposals/design/runtime-package"
...
case_experience_product_scaffold_passes() {
  local fixture_root output bundle_root package_root
  fixture_root="$(new_fixture_repo)"
  output="$(run_create_workflow "$fixture_root" experience-package "Experience Package" experience-product ".octon/framework/scaffolding/runtime/example.md")"
  bundle_root="$(printf '%s\n' "$output" | sed -n 's/^bundle_root: //p' | tail -n 1)"
  package_root="$fixture_root/.octon/inputs/exploratory/proposals/design/experience-package"
...
case_duplicate_package_failure_writes_receipts() {
  ...
  fixture_root="$(new_fixture_repo)"
  first_output="$(run_create_workflow "$fixture_root" duplicate-package "Duplicate Package" domain-runtime ".octon/framework/orchestration/runtime/example.md")"
  [[ -n "$first_output" ]] || return 1

  if output="$(run_create_workflow "$fixture_root" duplicate-package "Duplicate Package" domain-runtime ".octon/framework/orchestration/runtime/example.md" 2>&1)"; then
    return 1
  fi

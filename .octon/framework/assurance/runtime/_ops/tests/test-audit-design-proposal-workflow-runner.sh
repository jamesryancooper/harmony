...
# Fixture Design Proposal

This is a minimal fixture proposal for design-proposal workflow runner tests.
EOF
...
run_runner() {
  ...
  local slug="$4"
  shift 4
  "$RUNNER" workflow run audit-design-proposal \
    --set "proposal_path=$proposal_rel" \
    --set "mode=$mode" \
    --executor "$executor" \
    --output-slug "$slug" \
    "$@"
...
case_short_mock_runner_passes() {
  ...
  [[ "$bundle_root" != *"/.octon/state/evidence/validation/audits/"* ]] || return 1
  ...
  assert_file_exists "$bundle_root/reports/02-design-proposal-remediation.md" || return 1
  ...
  grep -Fq "CHANGE MANIFEST" "$bundle_root/reports/02-design-proposal-remediation.md" || return 1
  grep -Fq "stage 02 | executor=mock" "$bundle_root/commands.md" || return 1
  assert_file_exists "$bundle_root/stage-inputs/02-02-design-proposal-remediation.prompt.md" || return 1
  assert_file_exists "$bundle_root/stage-logs/02-02-design-proposal-remediation.log" || return 1
  assert_file_exists "$fixture_dir/.octon-mock-runner/synthetic-remediation.md" || return 1
}
...
case_live_runner_optional() {
  if [[ "${OCTON_RUN_LIVE_EXECUTOR_SMOKE:-0}" != "1" ]]; then
  ...
  fixture_dir="$(new_fixture_package)"
  fixture_rel="${fixture_dir#$ROOT_DIR/}"
  executor="${OCTON_LIVE_EXECUTOR:-codex}"
...
main() {
  if case_short_mock_runner_passes; then
    pass "audit-design-proposal workflow runner short-mode mock execution"
  else
    fail "audit-design-proposal workflow runner short-mode mock execution"
  fi

  if case_rigorous_mock_runner_passes; then
    pass "audit-design-proposal workflow runner rigorous-mode mock execution"
  else
    fail "audit-design-proposal workflow runner rigorous-mode mock execution"
  fi

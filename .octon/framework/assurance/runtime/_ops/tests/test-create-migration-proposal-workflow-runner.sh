...
new_fixture_repo() {
  mkdir -p "$TMP_ROOT"
  local fixture_root
  fixture_root="$(mktemp -d "$TMP_ROOT/create-migration-proposal.XXXXXX")"
...
run_create_workflow() {
  ...
  (
    cd "$fixture_root"
    "$RUNNER" workflow run create-migration-proposal \
...
case_scaffold_passes() {
  ...
  output="$(run_create_workflow "$fixture_root" runtime-package "Runtime Migration Proposal" octon-internal ".octon/framework/orchestration/runtime/example.md")"
  bundle_root="$(printf '%s\n' "$output" | sed -n 's/^bundle_root: //p' | tail -n 1)"
  proposal_root="$fixture_root/.octon/inputs/exploratory/proposals/migration/runtime-package"

  [[ "$bundle_root" == *"/.octon/state/evidence/runs/workflows/"* ]] || return 1
  assert_dir_exists "$proposal_root" || return 1
  assert_file_exists "$proposal_root/proposal.yml" || return 1
  assert_file_exists "$proposal_root/migration-proposal.yml" || return 1
  ...
  grep -Fq 'runtime-package' "$fixture_root/.octon/generated/proposals/registry.yml" || return 1
}

case_repo_local_scaffold_passes() {
  local fixture_root output proposal_root
  fixture_root="$(new_fixture_repo)"
  output="$(run_create_workflow "$fixture_root" repo-local-proposal "Repo Local Migration Proposal" repo-local "CHANGELOG.md")"
  proposal_root="$fixture_root/.octon/inputs/exploratory/proposals/migration/repo-local-proposal"
...
case_duplicate_id_fails() {
  ...
  fixture_root="$(new_fixture_repo)"
  output="$(run_create_workflow "$fixture_root" duplicate-proposal "Duplicate Proposal" octon-internal ".octon/framework/orchestration/runtime/example.md")"
  [[ -n "$output" ]] || return 1
  if run_create_workflow "$fixture_root" duplicate-proposal "Duplicate Proposal" octon-internal ".octon/framework/orchestration/runtime/example.md" >/dev/null 2>&1; then
    return 1
  fi
  return 0
}
...
main() {
  case_scaffold_passes && pass "create-migration-proposal scaffolds a migration proposal with workflow bundle receipts" || fail "create-migration-proposal scaffolds a migration proposal with workflow bundle receipts"
  case_repo_local_scaffold_passes && pass "create-migration-proposal scaffolds a repo-local proposal" || fail "create-migration-proposal scaffolds a repo-local proposal"
  case_duplicate_id_fails && pass "create-migration-proposal duplicate proposal failure writes receipts" || fail "create-migration-proposal duplicate proposal failure writes receipts"
  echo

...
- Persist durable runtime state under `.octon/state/control/execution/kv/`.
...
- `.octon/framework/capabilities/runtime/services/manifest.runtime.yml` - Tier 1 discovery (`interfaces/kv`).
- `.octon/framework/capabilities/runtime/services/registry.runtime.yml` - Tier 2 metadata (owner, risk, state path).
...
- State directory: `.octon/state/control/execution/kv`.
- Manifest limits:
  - `max_concurrency`: 4
  - `timeout_ms`: 30000
...
- Host persistence is a JSON map at `.octon/state/control/execution/kv/store.json`.

## Development

From `.octon/framework/capabilities/runtime/services/interfaces/kv/rust/`:

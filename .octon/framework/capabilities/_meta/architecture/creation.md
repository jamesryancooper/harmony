...
This invokes the `create-skill` skill defined in `.octon/framework/capabilities/runtime/skills/meta/create-skill/`.
...
| **Copy Template** | Copy `_scaffold/template/` to `.octon/framework/capabilities/runtime/skills/<group>/<skill-name>/` |
...
Add the skill to `.octon/framework/capabilities/runtime/skills/manifest.yml` for Tier 1 discovery:
...
Add extended metadata to `.octon/framework/capabilities/runtime/skills/registry.yml`:
...
If the skill needs harness-specific I/O paths, add them to `.octon/framework/capabilities/runtime/skills/registry.yml`:

...
create_fixture_repo() {
  ...
  local -a required_files=(
    ...
    ".octon/framework/capabilities/governance/policy/acp-operation-classes.md"
    ".octon/framework/capabilities/governance/policy/flags.metadata.json"
    ".octon/framework/capabilities/governance/policy/flags.metadata.schema.json"
    ".octon/framework/capabilities/_ops/scripts/policy-receipt-write.sh"
    ".octon/framework/capabilities/_ops/scripts/policy-circuit-breaker-actions.sh"
    ".octon/framework/capabilities/_ops/scripts/validate-flag-metadata.sh"
...
case_detects_affirmative_legacy_terms() {
  ...
  mkdir -p "$fixture_root/.octon/framework/cognition/governance/principles"
  cat > "$fixture_root/.octon/framework/cognition/governance/principles/deny-by-default.md" <<'EOF'
...
case_detects_pre_promote_human_approval_language() {
  ...
  mkdir -p "$fixture_root/.octon/framework/cognition/practices/methodology"
  cat > "$fixture_root/.octon/framework/cognition/practices/methodology/gates.md" <<'EOF'
...
case_allows_explicit_negation_terms() {
  ...
  mkdir -p "$fixture_root/.octon/framework/cognition/governance/principles"
  cat > "$fixture_root/.octon/framework/cognition/governance/principles/autonomous-control-points.md" <<'EOF'
...
case_detects_tracked_temp_artifacts() {
  ...
  mkdir -p "$fixture_root/.octon/state/evidence/validation/.tmp"
  printf 'temp artifact\n' > "$fixture_root/.octon/state/evidence/validation/.tmp/fixture.tmp"
  (
    cd "$fixture_root"
    git add ".octon/state/evidence/validation/.tmp/fixture.tmp"
  )
...
main() {
  ...
  assert_failure_contains \
    ...
    "stale legacy HITL language remains on active .octon surfaces" \
    case_detects_affirmative_legacy_terms

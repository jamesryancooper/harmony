...
usage() {
  cat <<'EOF'
Usage:
  .octon/framework/capabilities/_ops/scripts/validate-deny-by-default.sh [options]
...
collect_changed_targets() {
  ...
  while IFS= read -r changed_file; do
    ...
    if [[ "$changed_file" == .octon/framework/capabilities/runtime/services/* ]]; then
      local service_path
    ...
    if [[ "$changed_file" == .octon/framework/capabilities/runtime/skills/* ]]; then
      local skill_path
    ...
    case "$changed_file" in
      ...
      .octon/framework/capabilities/runtime/skills/_ops/scripts/*)

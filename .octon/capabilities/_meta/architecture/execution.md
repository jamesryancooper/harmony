...
outputs:
  - .octon/scaffolding/practices/prompts/20250115-refined.md
tools_used:
  - filesystem.read
  - filesystem.write
---
...
| Tier | Scope | Purpose |
|------|-------|---------|
| **Tier 1** | `.octon/{{category}}/` | Standard deliverables (final destination) |
| **Tier 2** | `.octon/**` | Custom harness locations (must declare) |
...
> **Note:** All `.octon/capabilities/runtime/skills/` categories follow the `{{category}}/{{skill-id}}/` pattern. Skills typically read from `_ops/state/configs/` and `_ops/state/resources/`, and write to `_ops/state/runs/` and `_ops/state/logs/`.
...
|----------|--------------|
| Prompts | `.octon/scaffolding/practices/prompts/{{timestamp}}-{{name}}.md` |
| Drafts | `.octon/output/drafts/{{timestamp}}-{{name}}.md` |
| Reports | `.octon/output/reports/analysis/{{timestamp}}-{{name}}.md` |
...
|---------|---------|---------|
| **Path interpolation** | Runtime path resolution in registry | `.octon/{{category}}/{{timestamp}}-{{name}}.md` |
...
**Result:** `.octon/output/drafts/api-design-synthesis.md`

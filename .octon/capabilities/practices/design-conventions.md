...
|-----------|--------------|-----------------|
| `.octon/capabilities/runtime/skills/` | Skill-first | **Authoring** — work on one skill at a time |
| `.octon/capabilities/runtime/skills/` | Artifact-first | **Operations** — debug, clean up, monitor across skills |
...
```markdown
.octon/capabilities/runtime/skills/_ops/state/runs/refactor/2026-01-19-rename-scratch/
...
4. **Document your settings** — Override in `.octon/capabilities/runtime/skills/_ops/state/configs/defaults.yml`:

```yaml
# .octon/capabilities/runtime/skills/_ops/state/configs/defaults.yml
scope_limits:
...
See `.octon/orchestration/runtime/workflows/` for mission templates and orchestration patterns.
...
2. Check against `.octon/cognition/runtime/context/continuity.md` if it exists
3. If match found:
...
|----------|---------|--------------|
| `prompts` | Refined prompts | `.octon/scaffolding/practices/prompts/` |
| `drafts` | Document drafts | `.octon/output/drafts/` |
| `reports` | Analysis reports | `.octon/output/reports/analysis/` |
| `analyses` | Code/data analyses | `.octon/cognition/runtime/analyses/` |
...
|------|-------|--------------|----------|
| **Tier 1** | `.octon/{{category}}/` | `.octon/scaffolding/practices/prompts/refined.md` | Standard deliverables |
...
skills:
  generate-client:
    outputs:
      ...
      - path: ".octon/output/reports/analysis/{{{run-id}}}-generation.md"
        type: deliverable
...
#### Operational Artifacts (`.octon/capabilities/runtime/skills/`)
...
**Decision:** [D040](/.octon/cognition/runtime/context/decisions.md) — Skills can fetch rule sets from external URLs at runtime using the `external-dependent` capability. This keeps audits current without harness updates, at the cost of requiring network access.

**Canonical example:** [`audit-ui`](/.octon/capabilities/runtime/skills/audit/audit-ui/SKILL.md) — fetches web design guidelines from an external URL, scans UI files, and reports violations.

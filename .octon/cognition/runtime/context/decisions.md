---
title: Decisions
description: Generated ADR summary for cognition runtime decision discovery.
mutability: generated
generated_from:
  - ../decisions/index.yml
  - ../decisions/*.md
---

# Decisions

This file is generated by `/.octon/cognition/_ops/runtime/scripts/sync-runtime-artifacts.sh`.
Do not edit manually. Update ADR files and `runtime/decisions/index.yml` instead.

## ADR Catalog

| ADR | Status | Date | Title | Path |
|---|---|---|---|---|
| ADR-001 | superseded | 2026-01-13 | ADR-001: Shared .octon/ Foundation | `../decisions/001-octon-shared-foundation.md` |
| ADR-002 | accepted | 2026-01-13 | ADR-002: Consolidated .scratchpad/ Human-Led Zone | `../decisions/002-consolidated-scratchpad-zone.md` |
| ADR-003 | accepted | 2026-01-14 | ADR-003: Projects Elevation and Idea Funnel | `../decisions/003-projects-elevation-and-funnel.md` |
| ADR-004 | accepted | 2026-01-14 | ADR-004: Refactor Workflow and Universal Commands | `../decisions/004-refactor-workflow.md` |
| ADR-005 | accepted | 2026-01-14 | ADR-005: Workflow Meta-Architecture and Gap Remediation | `../decisions/005-workflow-meta-architecture.md` |
| ADR-006 | accepted | 2026-01-14 | ADR-006: Prompt Refiner Skill | `../decisions/006-prompt-refiner-skill.md` |
| ADR-007 | accepted | 2026-01-14 | ADR-007: Primitives Documentation | `../decisions/007-primitives-documentation.md` |
| ADR-008 | superseded | 2026-01-15 | ADR-008: Skills Architecture Refactor | `../decisions/008-skills-architecture-refactor.md` |
| ADR-009 | accepted | 2026-01-17 | ADR-009: Manifest-Based Discovery and Validation Tooling | `../decisions/009-manifest-discovery-and-validation.md` |
| ADR-010 | accepted | 2026-02-07 | ADR-010: Single-Root Capability-Organized Structure | `../decisions/010-single-root-capability-structure.md` |
| ADR-011 | accepted | 2026-02-11 | ADR-011: Agency Subsystem Finalization | `../decisions/011-agency-subsystem-finalization.md` |
| ADR-012 | accepted | 2026-02-14 | ADR-012: Agent Platform Interop Native-First Contract | `../decisions/012-agent-platform-interop-native-first.md` |
| ADR-013 | accepted | 2026-02-15 | ADR-013: Filesystem Interfaces Native-First Interface | `../decisions/013-filesystem-interfaces-native-first.md` |
| ADR-014 | accepted | 2026-02-18 | ADR 014: Two-File Assurance Weight Governance Model | `../decisions/014-two-file-weight-governance-model.md` |
| ADR-015 | accepted | 2026-02-18 | ADR 015: Add Autonomy as a Quality Attribute | `../decisions/015-autonomy-quality-attribute.md` |
| ADR-016 | Accepted | unknown | ADR 016: Assurance Charter Integration in Assurance Engine | `../decisions/016-assurance-charter-assurance-engine-integration.md` |
| ADR-017 | Accepted | unknown | ADR 017: Assurance Clean-Break Migration | `../decisions/017-assurance-clean-break-migration.md` |
| ADR-018 | accepted | 2026-02-19 | ADR 018: Assurance Umbrella Chain Migration | `../decisions/018-assurance-umbrella-chain-migration.md` |
| ADR-019 | accepted | 2026-02-19 | ADR 019: Deny-by-Default Uniform Enforcement and Agent-Only Operation | `../decisions/019-deny-by-default-uniform-enforcement-and-agent-only-operation.md` |
| ADR-020 | Superseded by ADR 042 | 2026-02-20 | ADR 020: Engineering Principles Charter Successor v2026-02-20 | `../decisions/020-principles-charter-successor-v2026-02-20.md` |
| ADR-021 | accepted | 2026-02-20 | ADR-021: Bounded Surfaces Contract and Agency Clean-Break Migration | `../decisions/021-bounded-surfaces-contract-and-agency-migration.md` |
| ADR-022 | accepted | 2026-02-20 | ADR-022: Orchestration Bounded Surfaces Clean-Break Migration | `../decisions/022-orchestration-bounded-surfaces-clean-break-migration.md` |
| ADR-023 | accepted | 2026-02-20 | ADR-023: Capabilities Bounded Surfaces Clean-Break Migration | `../decisions/023-capabilities-bounded-surfaces-clean-break-migration.md` |
| ADR-024 | accepted | 2026-02-20 | ADR-024: Assurance Bounded Surfaces Clean-Break Migration | `../decisions/024-assurance-bounded-surfaces-clean-break-migration.md` |
| ADR-025 | accepted | 2026-02-20 | ADR-025: Scaffolding Bounded Surfaces Clean-Break Migration | `../decisions/025-scaffolding-bounded-surfaces-clean-break-migration.md` |
| ADR-026 | Accepted | 2026-02-20 | ADR 026: Engine Bounded Surfaces Clean-Break Migration | `../decisions/026-engine-bounded-surfaces-clean-break-migration.md` |
| ADR-027 | Accepted | 2026-02-20 | ADR 027: Cognition Bounded Surfaces Clean-Break Migration | `../decisions/027-cognition-bounded-surfaces-clean-break-migration.md` |
| ADR-028 | Accepted | 2026-02-21 | ADR 028: Agency Runtime Surface Clean-Break Rename | `../decisions/028-agency-runtime-surface-clean-break-rename.md` |
| ADR-029 | Accepted | 2026-02-21 | ADR 029: Quality-Gate Domain Split Clean-Break Migration | `../decisions/029-quality-gate-domain-split-clean-break-migration.md` |
| ADR-030 | Accepted | 2026-02-21 | ADR 030: Documentation Audit Clean-Break Rename | `../decisions/030-documentation-audit-clean-break-rename.md` |
| ADR-031 | Accepted | 2026-02-21 | ADR 031: Cognition Runtime Migrations Surface Split | `../decisions/031-cognition-runtime-migrations-surface-split.md` |
| ADR-032 | Accepted | 2026-02-21 | ADR 032: Migration Evidence Bundle Format | `../decisions/032-migration-evidence-bundle-format.md` |
| ADR-033 | Accepted | 2026-02-21 | ADR 033: ADR Discovery and Evidence Surface | `../decisions/033-adr-discovery-and-evidence-surface.md` |
| ADR-034 | accepted | 2026-02-16 | ADR-034: Planning+Execution Services Native-First, No-Python Core Runtime | `../decisions/034-planning-services-native-first-no-python.md` |
| ADR-035 | accepted | 2026-02-21 | ADR-035: Cognition Discovery and Governance Hardening | `../decisions/035-cognition-discovery-and-governance-hardening.md` |
| ADR-036 | accepted | 2026-02-21 | ADR-036: Cognition Sidecar Section Index Architecture | `../decisions/036-cognition-sidecar-section-index-architecture.md` |
| ADR-037 | Accepted | 2026-02-22 | ADR 037: Artifact Surface Clean-Break Rename | `../decisions/037-artifact-surface-clean-break-rename.md` |
| ADR-038 | Accepted | 2026-02-24 | ADR 038: Engine and Capabilities Authority Boundary | `../decisions/038-engine-capabilities-authority-boundary.md` |
| ADR-039 | Accepted | 2026-02-24 | ADR 039: Clean-Break Governance Cutover Contract | `../decisions/039-clean-break-governance-cutover-contract.md` |
| ADR-040 | Superseded by ADR 042 | 2026-02-24 | ADR 040: Principles Charter Successor v2026-02-24 | `../decisions/040-principles-charter-successor-v2026-02-24.md` |
| ADR-041 | Superseded by ADR 042 | 2026-02-24 | ADR 041: Principles Major Framing Human Override Contract | `../decisions/041-principles-major-framing-human-override-contract.md` |
| ADR-042 | Accepted | 2026-02-24 | ADR 042: Principles Charter Human-Override Direct-Edit Policy | `../decisions/042-principles-charter-human-override-direct-edit-policy.md` |
| ADR-043 | Accepted | 2026-02-24 | ADR 043: Principles Charter Override Enforcement and Audit | `../decisions/043-principles-charter-override-enforcement-and-audit.md` |
| ADR-044 | Accepted | 2026-02-25 | ADR 044: Intent Contract and Boundary Enforcement | `../decisions/044-intent-contract-and-boundary-enforcement.md` |
| ADR-045 | Accepted | 2026-02-25 | ADR 045: Capability Map and Alignment Drift Gates | `../decisions/045-capability-map-and-alignment-drift-gates.md` |
| ADR-046 | Accepted | 2026-03-04 | ADR 046: Execution-Profile Governance Cutover Contract | `../decisions/046-execution-profile-governance-cutover-contract.md` |
| ADR-047 | Accepted | 2026-03-06 | ADR 047: Self-Contained Bootstrap And Ingress Adapters | `../decisions/047-self-contained-bootstrap-and-ingress-adapters.md` |

## Update Procedure

1. Add or update ADR files in `/.octon/cognition/runtime/decisions/`.
2. Update `/.octon/cognition/runtime/decisions/index.yml`.
3. Run `bash .octon/cognition/_ops/runtime/scripts/sync-runtime-artifacts.sh`.

...
This prompt is **stage 1 of 4** in the Octon concept-integration prompt set.
...
Do **not** treat this extraction output alone as the final integration basis when the full 4-step pipeline is being used.
...
You must inspect the live Octon repository before making any current-state
claim.

Start with the current authority and topology anchors unless the user gives a
...
14. any repo surfaces directly implicated by the candidate concept

Do not infer repository reality from stale assumptions, earlier research runs,
old proposal packets, or this prompt alone.

If the live repo diverges from the assumptions below, record a
**Repository Drift Note** and proceed from the observed repository state.

The current checked-out repo outranks stale summaries and prior expectations.

---

## Octon-Specific Operating Assumptions

When evaluating fit, assume the following are binding unless live repository
inspection proves divergence.
...
- Conversation history, UI state, labels, comments, checks, and generated
  summaries are advisory unless materialized into canonical authority or
  evidence surfaces.

### 3. Execution, autonomy, and disclosure model

Octon's current live model is run-contract-centered, fail-closed, and
evidence-backed.

That means:

- the workspace charter pair is the active repo-wide objective layer beneath
  the constitutional kernel and repo governance declarations
- mission authority remains the continuity and long-horizon autonomy surface,
  not the atomic consequential execution unit
- consequential execution centers on bound run contracts and run-lifecycle
  control under `state/control/execution/**`
- approvals, exceptions, revocations, and other mutable execution control truth
  must materialize into canonical control surfaces
- retained run evidence, control-plane mutation evidence, lab evidence,
  publication receipts, and disclosure artifacts belong under
  `state/evidence/**`
- RunCard, HarnessCard, and other disclosure outputs are downstream of durable
  authority and retained evidence, not substitutes for them
- side effects still cross the engine-owned authorization boundary
- silence is not consent

### 4. Support-universe and adapter model

- Live support claims are bounded by
  `instance/governance/support-targets.yml` plus
  `instance/governance/exclusions/action-classes.yml`.
- Concepts that widen support claims must be treated as governance-impacting
  and require support-target, dossier, validation, and disclosure implications
  to be called out explicitly.
- Host adapters and model adapters are replaceable, non-authoritative
  boundaries.
- Host signals may mirror status, but they never mint authority.

### 5. Placement and overlay constraints

- Repo-specific overlay authority is legal only where the framework declares an
  overlay point and the instance manifest enables it.
- Proposal packets are exploratory lineage under
  `inputs/exploratory/proposals/**`; they are not durable authority and must
  point promotion targets at surfaces outside the proposal tree.
- Derived outputs are legal only after the authoritative, control, evidence,
  and continuity surfaces they depend on already exist elsewhere.

### 6. Non-negotiable anti-patterns

Reject or heavily down-rank any concept that would:
...
- widen support claims without explicit support-target, evidence, and
  disclosure treatment
- let host or model adapters mint authority
- reintroduce descendant-local `.octon/` roots or ancestor-chain locality
  models
- collapse run-contract, stage-attempt, evidence, or disclosure gates into an
  ungoverned shortcut
- smuggle policy through practices or convenience docs
- introduce shadow governance, shadow memory, shadow orchestration, or shadow
  approvals
- treat proposal packets, generated summaries, or exploratory notes as if they
  were implemented capability

### 7. Preferred translation style

Prefer concepts that can be encoded as:
...
- control records
- evidence or disclosure artifacts
- repo-legible skills and scaffolds
- validators, tests, lints, or checks
- evaluation methods and retained receipts
- workflow or orchestration contracts
- proposal-first changes with explicit durable promotion targets

...
An exhaustive, bounded audit of the `.harmony/capabilities/skills/` subsystem across 14 dimensions: structural integrity, schema conformance, single-source-of-truth enforcement, capability-reference coherence, token budgets, trigger quality, security and safety, cross-skill dependencies, lifecycle and versioning, logging and runtime artifacts, documentation completeness, validator alignment, foundation hierarchy, and template completeness. The audit produces a findings report with severity-graded issues and actionable remediations.

...
- `.harmony/orchestration/workflows/` — workflow files referencing skills paths
- `.harmony/cognition/` — analyses and context files referencing skills

**Out of scope:**

- Runtime execution correctness (whether skills produce correct output when invoked)
- Archived skills (`.harmony/capabilities/skills/archive/`)
- Historical audit reports in `.harmony/output/reports/` (frozen artifacts)
...
Grep the entire project for old-style paths. Each pattern must return 0 matches (excluding `archive/` and `.harmony/output/reports/`):
...
2. Confirm paths resolving outside `.harmony/` are rejected
...
2. Every "Extension" claim is documented as a Harmony extension

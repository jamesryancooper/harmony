...
1. **Fix `PROJECT_ROOT` resolution (line 13):**
   ...
   # Repo root is 3 levels above skills dir: .harmony/capabilities/skills/ → repo
...
# Expected: -> ../../.harmony/capabilities/skills/meta/create-skill
```
...
- `.harmony/capabilities/skills/meta/create-skill/SKILL.md` (lines 73-74)
- `.harmony/capabilities/skills/meta/create-skill/references/io-contract.md` (lines 32, 52, 78)
...
grep -r '{{skill-name}}\|{{run-id}}' .harmony/capabilities/skills/meta/create-skill/
# Expected: 0 matches
```
...
- `.harmony/orchestration/workflows/meta/create-skill(x)/00-overview.md`
- `.harmony/orchestration/workflows/meta/create-skill(x)/02-copy-template.md`
- `.harmony/orchestration/workflows/meta/create-skill(x)/03-initialize-skill.md`
- `.harmony/orchestration/workflows/meta/create-skill(x)/06-report-success.md`
...
| File | Line(s) | Change |
|------|---------|--------|
| `.harmony/cognition/analyses/workflows-vs-skills-analysis.md:190` | `skill_mappings:` → `skills:` in YAML example | Replace with current registry schema (`skills.<id>.io`) |
...
- Need <specific scaffolding> following Harmony conventions
...
**File:** `.harmony/capabilities/skills/manifest.yml`

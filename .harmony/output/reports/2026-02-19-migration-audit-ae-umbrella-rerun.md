...
- **Evidence:**
  ...
  - `cargo test --manifest-path .harmony/runtime/crates/Cargo.toml -p harmony_assurance_tools` runs successfully with **`running 0 tests`**.
...
- **Recommendation:** Add `assurance_tools` unit/integration tests and fixture goldens, then enforce them in CI (`cargo test -p harmony_assurance_tools`).
...
- `.harmony/assurance/standards/weights/weights.yml`
- `.harmony/runtime/crates/assurance_tools/src/main.rs`
- `.github/workflows/assurance-weight-gates.yml`
- `.harmony/output/assurance/effective/repo-harmony__run-mode-ci__maturity-beta__profile-ci-reliability.md`
- `.harmony/output/assurance/results/repo-harmony__run-mode-ci__maturity-beta__profile-ci-reliability.md`
- `.harmony/output/assurance/policy/deviations/repo-harmony__run-mode-ci__maturity-beta__profile-ci-reliability.md`
...
2. **Blind-spot probe:** Broader counter-example scans across `.harmony` + `.github` found legacy tokens only in ADR/migration-report context files.
...
1. Create `.harmony/runtime/crates/assurance_tools/tests/` with umbrella-chain unit + fixture tests.
2. Add fixture directory `.harmony/runtime/crates/assurance_tools/tests/fixtures/umbrella/`.
3. Update `.github/workflows/assurance-weight-gates.yml` to run `cargo test -p harmony_assurance_tools`.

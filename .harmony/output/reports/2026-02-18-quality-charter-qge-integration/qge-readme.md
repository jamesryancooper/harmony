...
The weighted-quality policy and enforcement stack in this subsystem is called
the **Quality Governance Engine (QGE)**.

QGE is the harness **authoritative local engine** for quality governance:
...
- Evidence outputs are generated as local artifacts under `.harmony/output/quality/`.
...
QGE is charter-led and runs as a deterministic chain:
...
4. `_ops/scripts/compute-quality-score.sh` resolves effective weights and results.
5. `_ops/scripts/quality-gate.sh` enforces policy, charter alignment, and drift checks.
6. Generated artifacts are written under `../output/quality/`.

## Contents

| File | Purpose | When to Use |
|------|---------|-------------|
| `CHARTER.md` | Canonical QGE charter and source-of-truth priority chain | Before changing weights, gates, or policy decisions |
...
| `weights/weights.md` | Human-readable weighted quality profiles and governance notes | When reviewing/changing quality priorities |
| `weights/weights.yml` | Machine-readable policy weights source of truth | When changing quality policy and precedence overrides |
...
| `_ops/scripts/compute-quality-score.sh` | Computes effective weights + weighted scorecards | Local/CI quality scoring runs |
| `_ops/scripts/quality-gate.sh` | Enforces weighted gate hard-fail/soft-warn rules | CI/local quality gate decision |
| `../output/quality/effective/*.md` | Resolved effective-weight matrix by context | After resolver runs |
| `../output/quality/results/*.md` | Weighted results + backlog drivers by context | After resolver runs |
| `../output/quality/policy/deviations/*.md` | Repo-over-subsystem override deviation report by context | After resolver runs |

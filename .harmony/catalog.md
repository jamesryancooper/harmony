...
|----------|-------------|
| [bootstrap/AGENTS.md](./scaffolding/runtime/bootstrap/AGENTS.md) | Canonical `/.harmony/AGENTS.md` bootstrap source |
...
| [harmony/](./scaffolding/runtime/templates/harmony/) | Base .harmony/ template (all harnesses inherit from this) |
| [harmony-docs/](./scaffolding/runtime/templates/harmony-docs/) | Scoped template for documentation areas (extends base) |
| [harmony-node-ts/](./scaffolding/runtime/templates/harmony-node-ts/) | Scoped template for Node.js/TypeScript packages (extends base) |
...
| Script | Description |
|--------|-------------|
| [scaffolding/runtime/_ops/scripts/init-project.sh](./scaffolding/runtime/_ops/scripts/init-project.sh) | Stable wrapper path for the canonical bootstrap generator that writes `.harmony`-local authored files plus root ingress adapters |
| [init.sh](./init.sh) | Health check: verifies required files/directories exist |

**Usage:** Run `.harmony/scaffolding/runtime/_ops/scripts/init-project.sh --list-objectives` to inspect common use cases, then run `.harmony/scaffolding/runtime/_ops/scripts/init-project.sh --objective <id>` from repo root (or use `/init`) for project bootstrap. This writes canonical authored files under `/.harmony/` and refreshes the root `AGENTS.md` and `CLAUDE.md` ingress adapters. Add `--with-boot-files` to generate `BOOT.md` and `BOOTSTRAP.md`. Run `.harmony/init.sh` from `.harmony/` for harness health checks.

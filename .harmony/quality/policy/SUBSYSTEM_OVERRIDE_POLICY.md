...
Ensure repo-level overrides that alter subsystem-provided quality weights are explicit, reviewable, and auditable, while preserving the resolver precedence contract:
...
This policy is enforced by the Quality Governance Engine (QGE), which is the
authoritative local engine for quality governance in Harmony.

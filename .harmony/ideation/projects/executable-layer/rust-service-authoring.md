...
This document covers **guest-side service development**: how to author, build, and deploy Harmony services as WebAssembly components using Rust and `cargo-component`.
...
Even though the component model supports rich typed interfaces, Harmony already has typed I/O at the harness level via `service.json` JSON Schemas. For v1, keep the component ABI stable and generic:
...
.harmony/capabilities/services/<category>/<service>/
  ...
  rust/
    ...
    src/lib.rs
    wit/
      world.wit          # copy of the canonical WIT from .harmony/runtime/wit/
```
...
```toml
[package]
name = "harmony-service-<category>-<name>"
version = "0.1.0"
edition = "2021"
...
This should be identical to the canonical WIT at `.harmony/runtime/wit/world.wit` (defined in [rust-kernel-reference.md §3](rust-kernel-reference.md)). The `service new` command copies it at scaffold time.
...
3. Write scaffold files:
   ...
   - `rust/wit/world.wit` (copy from `.harmony/runtime/wit/world.wit`)
...
.harmony/runtime/run tool kv get --json '{"key": "test"}'
```

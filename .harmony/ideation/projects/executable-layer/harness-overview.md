# The Harmony Portable Harness: Overview and Architecture

> **Reading context:** This document describes the *existing* Harmony harness — its structure, governance, discovery model, and architectural principles. It provides essential background for understanding the executable layer design documents in this bundle. If you already have access to the `.harmony/` directory, this summarizes what lives there; if you don't, this is your orientation.
>
...
The `.harmony/` directory is a **repo-wide agent harness** — a self-contained, metadata-driven operating system for AI-assisted software development. It is a structured collection of markdown files, YAML manifests, and scripts organized into nine domains:
...
The harness defines a complete governance layer: a `CONSTITUTION.md` with red lines, a delegation policy, a memory policy, per-agent execution contracts (`AGENT.md`) and identity contracts (`SOUL.md`), and an umbrella specification (`cognition/_meta/architecture/specification.md`) with numbered invariants (HARMONY-SPEC-001 through HARMONY-SPEC-801).
...
The `harmony.yml` manifest declares which paths are portable (copy them to bootstrap a new repo via `harmony init`) vs. project-specific state that stays local. It also declares human-led zones and resolution rules for framework/project overlaps.
...
The harness is pure files — markdown, YAML, shell scripts. No runtime dependency, no proprietary platform, no SDK. It works with Claude Code, Cursor, any AI IDE, or a bare terminal. The methodology is explicitly stack-, host-, and environment-agnostic. The `portable:` list in `harmony.yml` means you can seed a new repo with the framework in one copy operation.
...
The harness uses a **single `.harmony/` root** per scope, organized into nine top-level domains. There is no separation of "framework" vs. "project" at the directory level — that distinction is metadata-driven via `harmony.yml`. This was a deliberate architectural decision (replacing a prior two-root convention):

```
.harmony/
├── harmony.yml          <- portability manifest (what's framework vs. project-local)
...
The pattern is recursive. A descendant `.harmony/` can live in any subdirectory (e.g., `packages/auth/.harmony/`) to provide area-specific context. The **nearest-harness rule** applies: agents resolve to the closest `.harmony/` ancestor, with the root as fallback for anything not locally overridden. Descendant harnesses are intentionally minimal — they include only the subsystems they need.

---

### Portability Architecture

Portability is **metadata-driven, not structural** (HARMONY-SPEC-002). The `portable:` list in `harmony.yml` declares exactly which paths are framework assets that travel to new repos via `harmony init`. Everything else (continuity logs, missions, decisions, project context) is project-specific state that stays put.
...
- **Skills** — composite instruction sets following the agentskills.io spec with Harmony extensions (progressive disclosure, tool permission allowlists, run logging)
...
Everything routable uses a **four-tier discovery stack** (HARMONY-SPEC-003), so agents load only what they need:
...
Host-specific directories (`.cursor/commands/`, `.claude/commands/`) are **thin wrappers** that delegate to `.harmony/` paths. No harness-specific logic lives in workflows or skills. This means the same capability definitions work identically regardless of which IDE or agent runtime invokes them.

---

### The Meta-Pattern

The architecture follows a recursive documentation pattern: the code is the *what*; `.harmony/` is the *how* and *why* of working on that code. It formalizes the runbooks, playbooks, and institutional knowledge that effective teams maintain — but co-locates it with the code, structures it for machine consumption, and makes it durable across context windows and session boundaries.
...
Harmony's pillars are organized in three phases forming a complete feedback loop:

**PLAN Phase:**

1. **Direction through Validated Discovery** — Build the right thing because every feature is validated before investment.
2. **Focus through Absorbed Complexity** — Build features, not infrastructure — Harmony handles the rest.
...
| ID | Rule | Summary |
|---|---|---|
| HARMONY-SPEC-001 | Domain-Organized Harness Root | Root harness must remain domain-organized with explicit top-level domains |
...
| HARMONY-SPEC-010 | Documentation and Contract Coupling | Changes should update corresponding docs in the same change set |
| HARMONY-SPEC-011 | Project Bootstrap Initialization | Project-level bootstrap artifacts initialized via `/init` |
| HARMONY-SPEC-101 | Agency Contract Boundary | Agency defines actor taxonomy, invocation model, and delegation boundaries |

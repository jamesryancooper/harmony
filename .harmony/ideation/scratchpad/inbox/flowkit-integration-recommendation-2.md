...
- **Repo script**: [`package.json`](package.json) defines `flowkit:run` → `pnpm --filter @harmony/kits flowkit:run-config`.
...
- Optionally: generate TS/Py clients and have `@harmony/flowkit` use the generated TS client (or keep direct fetch but validate against the contract schema).

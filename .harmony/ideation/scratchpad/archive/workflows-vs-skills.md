...
If such a primitive is justified, define what capabilities it must offer that skills cannot. If not justified, recommend deprecating `.harmony/workflows/` in favor of the production-ready skills system.
...
| **workflows primitive** | A separate primitive (`.harmony/workflows/`) that would exist outside the skills specification |
...
> |--------|----------|--------|
> | **Skills** (`.harmony/skills/`) | Production-ready | Follows agentskills.io spec, documented architecture, working implementations |
> | **Workflows** (`.harmony/workflows/`) | Draft | Conceptual structure only, not fully designed or built |
>
> **Important:** Do not make assumptions about what workflows are or will be based on the current draft. The draft is incomplete and could change significantly. The current file structure in `.harmony/workflows/` should be treated as **exploratory scaffolding**, not a design commitment.
>
...
2. **Current Implementations:**
   - `.harmony/workflows/` — List all workflow directories
   - `.harmony/skills/` — List all skill directories
   - `.harmony/skills/manifest.yml` — Active skill index
   - `.harmony/skills/registry.yml` — Extended skill metadata
...
List each skill in `.harmony/skills/` with:
...
List what exists in `.harmony/workflows/` **without assuming intent**:
...
- `.harmony/skills/` — Current skill implementations
- `.harmony/skills/manifest.yml` — Current skill definitions
- `.harmony/workflows/` — Current workflow implementations

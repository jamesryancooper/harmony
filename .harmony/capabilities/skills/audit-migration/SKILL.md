...
allowed-tools: Read Glob Grep Write(../../output/reports/*) Write(logs/*)
---
...
```
/audit-migration manifest="migration: {name: 'restructure', mappings: [{old: '.workspace/', new: '.harmony/'}]}"
```

Or with a manifest file:

```
/audit-migration manifest=".harmony/migrations/restructure-manifest.yml"
```
...
See the companion workflow `orchestrate-audit` at `.harmony/orchestration/workflows/audit/orchestrate-audit/` for coordinated parallel execution.
...
- `logs/audit-migration/` — Execution logs with index

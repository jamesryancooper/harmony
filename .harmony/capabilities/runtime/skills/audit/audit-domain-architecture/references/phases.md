  ...
  phases:
    - name: "Configure"
      steps:
        ...
        - "Resolve target mode: observed (path exists) or prospective (path missing but valid .harmony domain target)"
...
- `prospective` - Domain path is missing but valid as a Harmony domain target.

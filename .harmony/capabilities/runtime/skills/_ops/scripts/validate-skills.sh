...
#   See: .harmony/capabilities/_meta/architecture/capabilities.md
#
...
HARMONY_DIR="$(cd "$CAPABILITIES_DIR/.." && pwd)"
...
validate_skill_io_scope() {
    ...
    while IFS= read -r output_path; do
        if [[ -n "$output_path" ]]; then
            local validation_result
            validation_result=$(validate_path_scope "$output_path" "$REPO_ROOT/.harmony/capabilities/runtime/skills/")
...
#
# See: .harmony/capabilities/_meta/architecture/reference-artifacts.md#complexity-budget
...
validate_aggregate_complexity() {
    ...
    if [[ $aggregate_tokens -le $AGGREGATE_STANDARD_BUDGET ]]; then
        ...
        log_info "  See: .harmony/capabilities/_meta/architecture/reference-artifacts.md#reducing-complexity"
...
validate_capability_heuristics() {
    ...
    if [[ "$should_add_caps" == "true" ]]; then
        ...
        log_info "  See: .harmony/capabilities/_meta/architecture/capabilities.md"
        return 1
    fi
...
validate_skill() {
    ...
    if [[ -f "$SKILLS_REGISTRY" ]]; then
        if ! check_io_mappings "$skill_id"; then
            ...
            log_info "    .harmony/capabilities/runtime/skills/registry.yml → skills.$skill_id.io"
    ...
    if [[ $tool_check_status -eq 0 ]]; then
        ...
        log_info "  See .harmony/capabilities/_meta/architecture/specification.md for allowed-tools format"
    fi
    ...
    if [[ $services_check_status -eq 0 ]]; then
        ...
        log_info "  See .harmony/capabilities/runtime/services/manifest.yml for valid service ids"
    fi
    ...
    if [[ "$VALIDATION_PROFILE" == "strict" ]]; then
        ...
        if [[ $version_status -eq 0 ]]; then
            ...
            log_info "  Update version in .harmony/capabilities/runtime/skills/registry.yml when making changes"
        fi

# Index — Indexing For Retrieval

Index creates deterministic indexes—both on-disk and in databases—that power Harmony's document-centric and hybrid retrieval flows. It leans on strict, mode-specific contracts so downstream services can trust artifact shape, metadata, and behavior.
...
- **Inputs:** Normalized chunks from Ingest with stable IDs and metadata
- **Outputs:** Audit-ready index directories designed for Query, Plan/Agent, and Harmony CI flows
...
- **Query:** consumes Index stores for retrieval
- **Plan / Agent:** orchestrate index build pipelines
- **Ingest:** supplies normalized Markdown and non-Markdown content
- **Dataset:** provides corpora definitions
- **Diagram:** renders hybrid link-graph views
...
- **Chunk‑first contract**
  - Preserve stable `chunk_id` from Ingest in all hybrid artifacts to support chunk‑granularity retrieval and citations in Query.
...
- **I/O**
  - Reads `ingest/**` chunks plus `ingest/tree_edges.jsonl` and `ingest/summaries.jsonl` when provided by Ingest.
...
- **I/O**
  - Reads normalized code records from Ingest or scans a repo directly (language filters).
  - Writes `code/**`: `symbols.jsonl`, `xref.jsonl`, `callgraph.jsonl`, and `files.jsonl` (optional previews).
- **Wins**
  - Precise, navigable code retrieval and evidence for refactors/migrations; grounds Dev/CodeMod plans.
- **Opinionated implementation choices**
...
- **I/O**
  - Reads tabular sources from Ingest (CSV/Parquet/SQL dumps).
...
Index's hybrid mode builds one or more retrieval signals and fuses them in Query. Enable signals with
`signals: ["dense", "keyword", "graph", "vision", "entities", "sparse_learned"]` and tune fusion via `fuse`.

Serving note: If you publish to a database (e.g., PostgreSQL + pgvector + FTS/trigrams + edges), Query can retrieve
directly from the DB instead of local artifacts. See [Query](../query/guide.md) (Database Adapters).
...
- Graph
  ...
  - Retrieval: Query expands neighborhoods up to `max_hops`, then fuses candidates with dense/keyword according
...
- Vision
  ...
  - Inputs: images, PDFs, or page snapshots normalized via Ingest (includes document URI linkage).
...
- Entities
  ...
  - Inputs: text from Ingest; optional dictionaries for ID resolution.
...
Index reinforces the Harmony methodology, as detailed in [the methodology overview](../../../../../cognition/practices/methodology/README.md):
...
**TL;DR:** Index is a "drop-in" Harmony knowledge indexer that keeps RAG pipelines fast, safe, and auditable.
...
- **Can it handle non-Markdown inputs?** Yes. Delegate normalization to Ingest; Index indexes the normalized output.
- **How are stable IDs enforced?** Prefer an `id` field in frontmatter; fall back to `path + content hash` for deterministic IDs.
- **How does document-centric RAG work here?** `document` mode emits a per-document store that Query ranks and hands off to an LLM.
...
- **Input contract (from Ingest)**
  - Required: `uri` (path or URL), `source` (namespace), `ts` (build timestamp), `hash` (content hash), `tokens` (int), `text` (string)
  - Recommended: `id` (stable string). If absent, Index derives a deterministic ID (e.g., `uri + content hash`). Ingest typically sets `id` from frontmatter when present. See [Ingest](../ingest/guide.md).
...
Index is the source of truth for reproducible, on‑disk snapshots. When you need serving features (filters/joins, shared access), add a publish step that upserts artifacts into a database. For small→mid corpora, a unified PostgreSQL stack works well:
...
- ParadeDB: If you adopt ParadeDB, use its search extensions to index `docs.text` and run BM25/semantic queries within PostgreSQL. Keep Index snapshots as your audit baseline.
...
- Security: keep DSNs in secrets (Vault); avoid committing credentials.
...
- Memory scales with embedding dim × doc chunks; tune chunking in Ingest and `min_tokens` here.
- BM25 `k1`/`b` tuning impacts sparse recall; start with defaults and adjust using Eval metrics.
...
- Eval hooks: consume Index artifacts in Eval to track Recall@k/MRR before and after changes.
...
- Log `schema_version`, build `ts`, corpus hash, mode/signal knobs, and artifact sizes to Observe for traceability.
...
- **Use in retrieval**: Query expands neighborhoods up to `max_hops`, then re‑ranks fused candidates.
...
- Stopwords/stemming by `lang`; if absent, default to `en`. Ingest may set `lang` during normalization.
- Preserve code blocks for preview but exclude from BM25 unless tagged (e.g., `labels: ["code:index"]`).
- Non‑text (images/PDFs) become text via Ingest extractors; retain `uri` and provenance.

## Security & Compliance

- Guard performs redaction before indexing; Index never writes secrets into artifacts.
...
- Query exposes runtime knobs like `nprobe` or `efSearch`; Index presets target build‑time memory/latency trade‑offs.
- For small corpora, use Query's in‑memory adapters; Index remains source of truth when you need persistence.
...
- Empty `links.jsonl`: enable link extraction in Ingest or add references; verify path normalization.
- Memory pressure on build: reduce chunk size in Ingest, lower embedding dim/model, or switch FAISS to `IVFFlat`.

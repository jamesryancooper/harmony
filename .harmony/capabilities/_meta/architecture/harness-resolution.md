...
RESOLVE_HARNESS(cwd, explicit_harness):
  ...
  2. Else → find the outermost `.harmony/` ancestor of cwd
...
Walk up from the starting directory and keep the outermost `.harmony/` ancestor:

```markdown
Starting: /repo/packages/kits/flowkit/src/
         ↑ collect any ancestor `.harmony/`
         /repo/.harmony/ → outermost match
...
If no repo-root `.harmony/` is found walking up from the starting point:
...
- **User action** — Create a repo-root `.harmony/` directory or specify `--harness` explicitly

...
usage() {
  cat <<'EOF'
Usage:
  .harmony/capabilities/_ops/scripts/validate-deny-by-default.sh [options]
...
run_service_validation() {
  ...
  HARMONY_VALIDATION_PROFILE="$PROFILE" "$SERVICES_VALIDATOR" --profile "$PROFILE" "$service_id"
}

run_skill_validation() {
  local skill_id="$1"
  echo "Validating skill policy: $skill_id"
  HARMONY_VALIDATION_PROFILE="$PROFILE" "$SKILLS_VALIDATOR" --profile "$PROFILE" "$skill_id"
}
...
main() {
  ...
  if [[ "$MODE" == "all" && ${#TARGET_SERVICES[@]} -eq 0 && ${#TARGET_SKILLS[@]} -eq 0 ]]; then
    HARMONY_VALIDATION_PROFILE="$PROFILE" "$SERVICES_VALIDATOR" --profile "$PROFILE" || status=$?
    HARMONY_VALIDATION_PROFILE="$PROFILE" "$SKILLS_VALIDATOR" --profile "$PROFILE" || status=$?
    if (( status == 0 )); then

...
This guide explains how Harmony protects against AI failures and what you should watch for when reviewing AI-generated content.
...
- Check imports are valid (run `harmony verify-imports` if unsure)
...
harmony check --guardrails

# Explain why AI made certain decisions
harmony explain "why did you use package X?"

# Retry with stricter constraints
harmony retry --constraint "only use packages already in package.json"
```
...
**What to do:** 
- Use `harmony retry --constraint "only implement login, nothing else"`
...
```bash
harmony retry --model claude-sonnet  # Try different model
harmony retry --context "do not use X, instead use Y"
```

### "AI output looks suspicious"

```bash
harmony verify-imports  # Check all imports are real
harmony check --guardrails  # Run full check
harmony explain "explain your reasoning"
```

### "Tests are failing on AI output"

```bash
harmony status  # Check what's failing
harmony retry --constraint "ensure all tests pass"
```
...
```bash
harmony retry --context "this is for internal testing purposes, not malicious"
```

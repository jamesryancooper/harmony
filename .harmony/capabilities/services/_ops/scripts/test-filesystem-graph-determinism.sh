#!/usr/bin/env bash
# test-filesystem-graph-determinism.sh - determinism and runtime-state exclusion checks.
...
build_snapshot() {
  "$RUNTIME_RUN" tool interfaces/filesystem-graph snapshot.build --json '{"root":".","state_dir":".harmony/runtime/_ops/state/snapshots","set_current":false}'
}
...
if [[ "$SNAP1" != "$SNAP2" ]]; then
  ...
  "$RUNTIME_RUN" tool interfaces/filesystem-graph snapshot.diff --json "$DIFF_INPUT"
  exit 1
fi
...
echo "filesystem-graph determinism passed: $SNAP1"

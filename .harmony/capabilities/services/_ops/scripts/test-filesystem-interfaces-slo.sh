...
FIXTURE_BUILDER="$HARMONY_DIR/capabilities/services/_ops/scripts/build-filesystem-interfaces-benchmark-fixture.sh"
SLO_BUDGETS="$HARMONY_DIR/capabilities/services/interfaces/filesystem-snapshot/contracts/slo-budgets.tsv"
...
tmp_dir="$HARMONY_DIR/output/reports/.tmp"
mkdir -p "$tmp_dir"
...
if [[ -z "$report_path" && "$emit_report" == "1" ]]; then
  report_path="$HARMONY_DIR/output/reports/$(date +%F)-filesystem-interfaces-slo-report.md"
fi

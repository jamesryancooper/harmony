...
CANONICAL_AGENTS_FILE="$HARMONY_DIR/AGENTS.md"
ROOT_AGENTS_FILE="$ROOT_DIR/AGENTS.md"
ROOT_CLAUDE_FILE="$ROOT_DIR/CLAUDE.md"
OBJECTIVE_FILE="$HARMONY_DIR/OBJECTIVE.md"
INTENT_FILE="$HARMONY_DIR/cognition/runtime/context/intent.contract.yml"
EXPECTED_INGRESS_TARGET=".harmony/AGENTS.md"
...
validate_ingress_adapter() {
  ...
  if [[ -f "$file_path" ]]; then
    if cmp -s "$CANONICAL_AGENTS_FILE" "$file_path"; then
      pass "$label fallback copy matches .harmony/AGENTS.md"
    else
      fail "$label fallback copy diverges from .harmony/AGENTS.md"
    fi
    return
  fi
...
validate_objective_headings() {
  ...
  for heading in "${required_headings[@]}"; do
    if rg -n "^## ${heading}$" "$OBJECTIVE_FILE" >/dev/null 2>&1; then
      pass ".harmony/OBJECTIVE.md contains section: $heading"
    else
      fail ".harmony/OBJECTIVE.md missing section: $heading"
    fi
  done
}
...
main() {
  ...
  if [[ -e "$ROOT_DIR/OBJECTIVE.md" || -L "$ROOT_DIR/OBJECTIVE.md" ]]; then
    fail "root OBJECTIVE.md must not exist; canonical objective brief lives at .harmony/OBJECTIVE.md"
  else
  ...
  [[ "$objective_schema" == "objective-brief-v1" ]] || fail ".harmony/OBJECTIVE.md schema_version must be objective-brief-v1"
  [[ "$intent_schema" == "intent-contract-v1" ]] || fail "intent contract schema_version must be intent-contract-v1"
  [[ -n "$objective_id" ]] || fail ".harmony/OBJECTIVE.md missing objective_id"
  [[ -n "$objective_intent_id" ]] || fail ".harmony/OBJECTIVE.md missing intent_id"
  [[ -n "$objective_intent_version" ]] || fail ".harmony/OBJECTIVE.md missing intent_version"
  [[ -n "$objective_owner" ]] || fail ".harmony/OBJECTIVE.md missing owner"
  [[ -n "$objective_approved_by" ]] || fail ".harmony/OBJECTIVE.md missing approved_by"

...
validate_promotion_targets() {
  ...
  local scope target_rel saw_harmony=0 saw_non_harmony=0 allow_legacy=0
  ...
  if [[ "$scope" == "harmony-internal" && "$saw_non_harmony" -eq 1 && "$allow_legacy" -ne 1 ]]; then
    fail "$label harmony-internal scope includes non-.harmony promotion targets"
  elif [[ "$scope" == "harmony-internal" ]]; then
    pass "$label harmony-internal targets stay under .harmony/"
  fi

  if [[ "$scope" == "repo-local" && "$saw_harmony" -eq 1 && "$allow_legacy" -ne 1 ]]; then
    fail "$label repo-local scope includes .harmony promotion targets"
  elif [[ "$scope" == "repo-local" ]]; then
    pass "$label repo-local targets stay outside .harmony/"
  fi

  if [[ "$saw_harmony" -eq 1 && "$saw_non_harmony" -eq 1 ]]; then
    if [[ "$allow_legacy" -eq 1 ]]; then
      warn "$label preserves historical mixed targets under legacy-unknown archive lineage"
    else
      fail "$label mixes .harmony and non-.harmony promotion targets"
    fi
  else
...
validate_proposal() {
  ...
  validate_enum "$scope" "proposal '$proposal_rel' scope valid" "harmony-internal" "repo-local"

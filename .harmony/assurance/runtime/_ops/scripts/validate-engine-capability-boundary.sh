...
CAPABILITIES_DIR="$HARMONY_DIR/capabilities"
COGNITION_OPS_DIR="$HARMONY_DIR/cognition/_ops"
SHIM_PATH="$CAPABILITIES_DIR/_ops/scripts/run-harmony-policy.sh"
...
check_policy_runner_cutover() {
  ...
  refs="$(rg -n --no-heading "run-harmony-policy\\.sh" \
    "$CAPABILITIES_DIR" \
    "$COGNITION_OPS_DIR" \
    --glob '!**/*.md' || true)"

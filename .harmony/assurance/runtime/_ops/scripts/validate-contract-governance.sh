...
CONTEXT_INDEX="$HARMONY_DIR/cognition/runtime/context/index.yml"
SPEC_FILE="$HARMONY_DIR/cognition/_meta/architecture/specification.md"
RUNTIME_OPS_CONTRACT="$HARMONY_DIR/cognition/_meta/architecture/runtime-vs-ops-contract.md"
...
REPORT_DIR="$HARMONY_DIR/output/assurance/results"
...
check_required_contract_text() {
  ...
  if grep -Fq "HARMONY-SPEC-015" "$SPEC_FILE"; then
    pass "umbrella specification contains HARMONY-SPEC-015"
  else
    fail "umbrella specification missing HARMONY-SPEC-015"
  fi

  if grep -Fq "HARMONY-SPEC-016" "$SPEC_FILE"; then
    pass "umbrella specification contains HARMONY-SPEC-016"
  else
    fail "umbrella specification missing HARMONY-SPEC-016"
  fi
...
check_ops_mutation_policy_metadata() {
  ...
  for key in allow_write_roots immutable_targets enforce_with; do
    ...
    while IFS= read -r rel; do
      [[ -z "$rel" ]] && continue
      found=1
      abs="$HARMONY_DIR/cognition/runtime/context/$rel"
...
check_contract_registry() {
  local context_dir="$HARMONY_DIR/cognition/runtime/context"
...
check_ops_boundary_violations() {
  ...
  done < <(find "$HARMONY_DIR" -path "*/_ops/*" -type f)

#!/usr/bin/env bash
# test-validate-orchestration-design-package.sh - Regression tests for design package validation.
...
main() {
  assert_success \
    "design package validator accepts the baseline proof layer" \
    case_valid_package_passes

  assert_failure_contains \
    "design package validator rejects missing schema artifacts" \
    "missing schema for schema-backed contract" \
    case_missing_schema_fails

  assert_failure_contains \
    "design package validator rejects missing validation coverage markers" \
    "required contract missing validation coverage marker" \
    case_missing_coverage_marker_fails

  assert_failure_contains \
    "design package validator rejects invalid fixtures that unexpectedly pass" \

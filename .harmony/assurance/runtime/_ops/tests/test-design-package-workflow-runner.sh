...
case_mock_runner_passes() {
  ...
  output="$(run_runner "$fixture_rel" mock design-package-runner-mock)"
...
case_live_runner_optional() {
  ...
  output="$(run_runner "$fixture_rel" "$executor" design-package-runner-live)"
...
main() {
  if case_mock_runner_passes; then
    pass "design-package workflow runner mock execution"
  else
    fail "design-package workflow runner mock execution"
  fi

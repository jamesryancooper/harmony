...
new_fixture_repo() {
  mkdir -p "$TMP_ROOT"
  local fixture_root
  fixture_root="$(mktemp -d "$TMP_ROOT/create-design-package.XXXXXX")"
...
run_create_workflow() {
  ...
  local package_id="$2"
  local package_title="$3"
  local package_class="$4"
  local implementation_targets="$5"
  (
    cd "$fixture_root"
    "$RUNNER" workflow run create-design-package \
      --set "package_id=$package_id" \
      --set "package_title=$package_title" \
      --set "package_class=$package_class" \
      --set "implementation_targets=$implementation_targets"
  )
}
...
case_domain_runtime_scaffold_passes() {
  ...
  package_root="$fixture_root/.design-packages/runtime-package"

  [[ "$bundle_root" == *"/.harmony/output/reports/workflows/"* ]] || return 1
  assert_dir_exists "$package_root" || return 1
  assert_file_exists "$package_root/design-package.yml" || return 1
  assert_file_exists "$fixture_root/.design-packages/registry.yml" || return 1
  ...
  grep -Fq 'runtime-package' "$fixture_root/.design-packages/registry.yml" || return 1
}
...
case_experience_product_scaffold_passes() {
  ...
  bundle_root="$(printf '%s\n' "$output" | sed -n 's/^bundle_root: //p' | tail -n 1)"
  package_root="$fixture_root/.design-packages/experience-package"

  assert_dir_exists "$package_root" || return 1
  assert_file_exists "$package_root/design-package.yml" || return 1
  assert_file_exists "$bundle_root/standard-validator.log" || return 1
  assert_file_exists "$bundle_root/bundle.yml" || return 1
  grep -Fq 'experience-package' "$fixture_root/.design-packages/registry.yml" || return 1
}
...
main() {
  if case_domain_runtime_scaffold_passes; then
    pass "create-design-package scaffolds a domain-runtime package with workflow bundle receipts"
  else
    fail "create-design-package scaffolds a domain-runtime package with workflow bundle receipts"
  fi

  if case_experience_product_scaffold_passes; then
    pass "create-design-package scaffolds an experience-product package with workflow bundle receipts"
  else
    fail "create-design-package scaffolds an experience-product package with workflow bundle receipts"
  fi

  if case_duplicate_package_failure_writes_receipts; then
    pass "create-design-package duplicate package failure writes classified bundle receipts"
  else
    fail "create-design-package duplicate package failure writes classified bundle receipts"
  fi

...
description: Keep Harmony self-contained and platform-neutral so core behavior runs without provider lock-in, runtime lock-in, or OS lock-in.
...
> Harmony core behavior must remain self-contained, tech-agnostic, and OS-agnostic by default.

## What This Means

Portability and Independence define three non-negotiable defaults for Harmony:

1. **Self-contained harness:** Required contracts, discovery metadata, and governance artifacts live inside `.harmony/` and do not rely on external repo structure.
...
Portability protects institutional memory from tool churn. Decisions encoded in Harmony remain usable even when teams, runtimes, or platforms change.

---
title: Risk Tiers
description: How Harmony classifies changes by risk, and what level of review you need to provide for each tier.
---
...
```bash
harmony tier-up <pr-number> --reason "touches user sessions"
```
...
3. **Approve spec to proceed**
   ```bash
   harmony approve-spec oauth-integration
   ```
...
harmony tier-up <id> --reason "touches sessions"

# Bump down (requires justification)
harmony tier-down <id> --reason "actually just a config change"
```
...
```bash
harmony tier-up <id> --reason "affects caching behavior"
```
...
```bash
harmony tier-down <id> --reason "config change only, no auth logic"
```

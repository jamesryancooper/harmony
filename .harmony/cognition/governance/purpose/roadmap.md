...
Welcome to the implementation roadmap for Harmony's Convivial Purpose. The foundational philosophy is established—the Resonant Computing Design Stack (RCDS) has been synthesized into Harmony's grounding statement, and the six pillars now reference this purpose as their *why*. The next phase focuses on operationalization: embedding Convivial Purpose into schemas, templates, guardrails, and measurement systems.
...
| Spec template CIA section | `.harmony/cognition/practices/methodology/templates/spec-tier*.yaml` | Not added |
...
```text
.harmony/cognition/governance/principles/purpose/

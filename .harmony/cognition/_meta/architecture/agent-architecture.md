...
description: Target architecture, repo layout, and access patterns for Harmony agents, aligned with the Harmony Structural Paradigm and methodology.
---
...
This document defines the **target architecture** for Harmony agents—how they are **conceptualized**, **structured in the monorepo**, and **consumed** by apps, Kaizen flows, and other tooling.
...
- **Guided Agentic Autonomy**: agents are explicitly governed by risk rubrics, ACP checkpoints, and fail‑closed defaults.
...
These gaps do not contradict Harmony Methodology, but they **increase cognitive load** and make it harder to:
...
The remainder of this document defines a **target agent architecture** that resolves these gaps while staying true to Harmony’s pillars.

## 2. Target Agent Model (Conceptual)

### 2.1 What Is an Agent in Harmony
...
- Must:
  - Satisfy Harmony’s **System Guarantees** (determinism, observability, rollback).
...
1. **Write the Agent Spec**
   - Create `packages/agents/src/specs/<agent-id>/spec.md` and (optionally) `spec.json`:
     - Define purpose, inputs/outputs, quality attributes, risk class, ACP checkpoints, and surfaces that will call this agent.
...
By following this structure, Harmony agents remain **spec‑first, deterministic, observable, testable, and reversible**, while staying decoupled from specific runtimes and easy to consume across any surface in the monorepo.

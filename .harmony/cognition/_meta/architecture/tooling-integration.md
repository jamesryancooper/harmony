...
Related docs: [monorepo polyglot (normative)](./monorepo-polyglot.md), [overview](./overview.md), [runtime architecture](./runtime-architecture.md), [knowledge plane](../../runtime/knowledge/knowledge.md), [observability requirements](./observability-requirements.md), [runtime policy](./runtime-policy.md), [contracts registry](./contracts-registry.md), [python runtime workspace example](/.harmony/scaffolding/practices/examples/stack-profiles/python-runtime-workspace.md)
...
Harmony uses a single Turborepo task graph that spans TypeScript and Python workspaces and enforces a contracts-first posture (see `monorepo-polyglot.md` for concrete `turbo.json`, `package.json`, `pnpm-workspace.yaml`, and `pyproject.toml` snippets).

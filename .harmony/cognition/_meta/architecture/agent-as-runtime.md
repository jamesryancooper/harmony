...
This document defines the architecture for running the Harmony harness (`.harmony/`) as pure declarative content interpreted by an AI agent. The goal is portability across repositories, stacks, and environments without shipping project-local executables or runtime binaries in the harness itself.
...
Harmony adopts an agent-as-runtime architecture for harness interpretation:
...
Harmony uses a two-tier validation model.
...
This aligns with [governance model](./governance-model.md) and Harmony principles on determinism, guardrails, and ACP checkpoints.

...
#[command(about = "Harmony deny-by-default policy engine")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
fn run_enforce(args: EnforceArgs) {
    ...
    let request = EnforceRequest {
        ...
        risk_tier: env_or_string(args.risk_tier, "HARMONY_RISK_TIER", "low"),
        agent_id: env_or_optional(args.agent_id, "HARMONY_AGENT_ID"),
        agent_ids_csv: env_or_optional(args.agent_ids, "HARMONY_AGENT_IDS"),
        review_agent_id: env_or_optional(args.review_agent_id, "HARMONY_REVIEW_AGENT_ID"),
        quorum_token: env_or_optional(args.quorum_token, "HARMONY_QUORUM_TOKEN"),
        rollback_plan_id: env_or_optional(args.rollback_plan_id, "HARMONY_ROLLBACK_PLAN_ID"),

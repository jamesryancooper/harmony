...
#[derive(Parser, Debug)]
#[command(name = "harmony-quality")]
#[command(about = "Harmony quality score and gate tools")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
#[derive(Args, Debug)]
struct ScoreArgs {
    #[arg(long, default_value = ".harmony/quality/weights/weights.yml")]
    weights: PathBuf,

    #[arg(long, default_value = ".harmony/quality/scores/scores.yml")]
    scores: PathBuf,

    #[arg(long, default_value = ".harmony/quality/CHARTER.md")]
    charter: PathBuf,

    #[arg(long, default_value = ".harmony/quality/weights/inputs/context.yml")]
    context: PathBuf,

    #[arg(long, default_value = ".harmony/quality/policy/subsystem-classes.yml")]
    subsystem_classes: PathBuf,

    #[arg(long, default_value = ".harmony/quality/policy/overrides.yml")]
    overrides: PathBuf,
    ...
    #[arg(long, default_value = ".harmony/output/quality/effective")]
    effective_dir: PathBuf,

    #[arg(long, default_value = ".harmony/output/quality/results")]
    results_dir: PathBuf,

    #[arg(long, default_value = ".harmony/output/quality/policy/deviations")]
    deviations_dir: PathBuf,

    #[arg(long, default_value = ".harmony/quality/_ops/state")]
    lock_dir: PathBuf,
}
...
fn run_score(args: ScoreArgs) -> Result<()> {
    ...
    scorecard.insert(
        ...
        Value::Object({
            ...
            m.insert(
                "tool".to_string(),
                Value::String("compute-quality-score".to_string()),
            );
            m.insert(
...
fn run_gate(args: GateArgs) -> Result<()> {
    ...
    let repo = get_string_obj(&context, "repo").unwrap_or_else(|| "harmony".to_string());
...
fn parse_subsystem_class_policy(input: Option<&Value>) -> Result<SubsystemClassPolicy> {
    ...
    subsystem_class.insert("quality".to_string(), "control-plane".to_string());
...
fn default_out_dir(now: &TimestampMeta) -> PathBuf {
    ...
    PathBuf::from(format!(
        ".harmony/output/quality/scorecards/{}/{}",
        day, now.run_id
    ))
}

...
pub fn service_new(harmony_dir: &Path, category: &str, name: &str) -> anyhow::Result<()> {
    ...
    let world_src = harmony_dir.join("runtime").join("wit").join("world.wit");
    ...
    let crate_name = format!("harmony-{}-{}-service", category, name);
...
"#;
    ...
    let service_json = serde_json::json!({
        ...
        "docs": {
            ...
            "help": "Generated by `harmony service new`"
        }
    });
...
pub fn service_build(harmony_dir: &Path, category: &str, name: &str) -> anyhow::Result<()> {
    validate_ident(category)?;
    validate_ident(name)?;

    let service_dir = harmony_dir
        .join("capabilities")
        .join("services")
        .join(category)
        .join(name);

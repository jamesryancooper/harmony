...
Initializes canonical Harmony bootstrap files and objective-contract artifacts.
USAGE
}
...
BOOTSTRAP_DIR="$HARMONY_DIR/scaffolding/runtime/bootstrap"
...
CANONICAL_AGENTS_OUT="$REPO_ROOT/.harmony/AGENTS.md"
...
OBJECTIVE_OUT="$REPO_ROOT/.harmony/OBJECTIVE.md"
...
CANONICAL_INGRESS_TARGET=".harmony/AGENTS.md"
...
write_canonical_agents() {
  ...
  if [[ "$existed" -eq 1 && "$FORCE" -ne 1 ]]; then
    echo "[SKIP] .harmony/AGENTS.md already exists: $CANONICAL_AGENTS_OUT"
    return
  fi
  ...
  if ! validate_generated_agents_file "$tmp_agents"; then
    rm -f "$tmp_agents"
    echo "[ERROR] Refusing to write non-compliant .harmony/AGENTS.md from template" >&2
    exit 1
  fi

  if [[ "$DRY_RUN" -eq 1 ]]; then
    if [[ "$existed" -eq 1 ]]; then
      echo "[DRY] Would overwrite .harmony/AGENTS.md from template: $CANONICAL_AGENTS_OUT"
    else
      echo "[DRY] Would create .harmony/AGENTS.md from template: $CANONICAL_AGENTS_OUT"
    fi
    rm -f "$tmp_agents"
    return
  fi
  ...
  if [[ "$existed" -eq 1 ]]; then
    echo "[OK] .harmony/AGENTS.md overwritten from template: $CANONICAL_AGENTS_OUT"
  else
    echo "[OK] .harmony/AGENTS.md created from template: $CANONICAL_AGENTS_OUT"
  fi
}
...
write_objective_contract() {
  ...
  write_objective_file "$objective_brief_template" "$OBJECTIVE_OUT" ".harmony/OBJECTIVE.md"

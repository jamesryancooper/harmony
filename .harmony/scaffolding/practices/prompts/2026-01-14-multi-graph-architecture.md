# Multi-Graph Agentic Memory Architecture for Harmony
...
## 1. Harmony Architecture Overview

Harmony uses a **two-layer inheritance model** for AI agent coordination:
...
pnpm harmony graph:query --node root:decision:D001

# Query: find all nodes of type
pnpm harmony graph:query --type decision

# Query: find what references a node
pnpm harmony graph:query --to root:decision:D001 --rel references

# Query: find what a node depends on
pnpm harmony graph:query --from api:task:T003 --rel depends_on

# Query: cross-graph traversal
pnpm harmony graph:traverse api:decision:API-001 --depth 2

# Watch mode (rebuild on file change)
pnpm harmony graph:watch
```
...
import { Graph, isStale } from '@harmony/kit-graph';
...
async function bootWithGraph(workspacePath: string) {
  ...
  if (await isStale(workspacePath)) {
    console.warn('Graph is stale. Run: pnpm harmony graph:rebuild');
  }
...
```typescript
import { FederatedGraph } from '@harmony/kit-graph';

const federated = await FederatedGraph.load('/path/to/harmony');
...
if pnpm harmony graph:status --quiet; then

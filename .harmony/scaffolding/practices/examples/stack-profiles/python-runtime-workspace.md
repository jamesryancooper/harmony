...
description: Non-normative stack-specific example for Python uv workspace and platform flow runtime design.
---
...
Related docs: [Monorepo polyglot (normative)](/.harmony/cognition/_meta/architecture/monorepo-polyglot.md), [Architecture overview](/.harmony/cognition/_meta/architecture/overview.md), [Monorepo layout](/.harmony/cognition/_meta/architecture/monorepo-layout.md), [Repository blueprint](/.harmony/cognition/_meta/architecture/repository-blueprint.md), [Tooling integration](/.harmony/cognition/_meta/architecture/tooling-integration.md), [Contracts registry](/.harmony/cognition/_meta/architecture/contracts-registry.md), [Agent roles](/.harmony/cognition/_meta/architecture/agent-roles.md), [Runtime architecture](/.harmony/cognition/_meta/architecture/runtime-architecture.md), [Runtime policy](/.harmony/cognition/_meta/architecture/runtime-policy.md)

This document provides an example Python runtime workspace profile for Harmony’s polyglot monorepo: how uv workspaces, `agents/*`, and the shared LangGraph-based **platform flow runtime service** under `platform/runtimes/flow-runtime/**` can fit together under a contracts-first, Turborepo+pnpm+uv model.

It is a **stack-specific example**, not a normative harness requirement. Any
`must`/`should` language below applies only if a project deliberately adopts
this profile.
...
For this profile, Python code in `agents/*` and
`platform/runtimes/flow-runtime/**` should use generated clients from
`contracts/py` for cross-service HTTP calls:
...
## Profile Assumptions vs Optional Choices

Assumed in this example profile:

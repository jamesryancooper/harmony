...
- `target-location`: `.harmony/orchestration/runtime/workflows/<domain>/` or `.harmony/orchestration/runtime/workflows/` (with `--local`)
- `domain`: Optional subdirectory under `.harmony/orchestration/runtime/workflows/`
...
2. **Determine target path:**
   ```text
   If --local flag:
     target = .harmony/orchestration/runtime/workflows/<workflow-id>/
   Else if --domain specified:
     target = .harmony/orchestration/runtime/workflows/<domain>/<workflow-id>/
   Else:

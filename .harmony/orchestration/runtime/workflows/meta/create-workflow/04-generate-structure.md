...
1. **Create workflow directory:**
   ```bash
   mkdir -p <target-path>
   # e.g., mkdir -p .harmony/orchestration/runtime/workflows/ci-cd/deploy-staging/
   ```

2. **Copy template files:**
   ```text
   For each file in template:
     Copy .harmony/orchestration/runtime/workflows/_scaffold/template/<file> to <target-path>/<file>
   ```
...
- Template not found: "Template file '<file>' not found in .harmony/orchestration/runtime/workflows/_scaffold/template/"

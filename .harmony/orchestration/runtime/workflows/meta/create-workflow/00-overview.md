...
- `.harmony/orchestration/runtime/workflows/<domain>/<workflow-id>/` for shared workflows
- `.harmony/orchestration/runtime/workflows/<workflow-id>/` for project-specific workflows (with `--local`)

## Prerequisites

- Workflow ID must be lowercase with hyphens (e.g., `code-review`)
- No existing workflow with the same ID in target location
- Template directory exists at `.harmony/orchestration/runtime/workflows/_scaffold/template/`
...
```text
.harmony/orchestration/runtime/workflows/<domain>/<workflow-id>/

...
AUDIT_SCRIPT=".harmony/orchestration/runtime/workflows/_ops/scripts/audit-workflow-system.sh"
VALIDATE_SCRIPT=".harmony/orchestration/runtime/workflows/_ops/scripts/validate-workflows.sh"
...
create_fixture_repo() {
  ...
  mkdir -p "$fixture_root/.harmony"
  cp -R "$REPO_ROOT/.harmony/orchestration" "$fixture_root/.harmony/"
  mkdir -p "$fixture_root/.harmony/cognition/runtime"
  cp -R "$REPO_ROOT/.harmony/cognition/runtime/context" "$fixture_root/.harmony/cognition/runtime/"
  cp -R "$REPO_ROOT/.harmony/cognition/runtime/audits" "$fixture_root/.harmony/cognition/runtime/"
  mkdir -p "$fixture_root/.harmony/assurance/runtime/_ops/scripts"
  cp "$REPO_ROOT/.harmony/assurance/runtime/_ops/scripts/alignment-check.sh" \
    "$fixture_root/.harmony/assurance/runtime/_ops/scripts/alignment-check.sh"
  mkdir -p "$fixture_root/.harmony/output/reports/audits" "$fixture_root/.harmony/output/.tmp"
  cp "$REPO_ROOT/.harmony/output/reports/audits/README.md" \
    "$fixture_root/.harmony/output/reports/audits/README.md"
...
case_score_directory_workflow() {
  (
    cd "$REPO_ROOT"
    bash "$AUDIT_SCRIPT" --mode score-workflow --target ".harmony/orchestration/runtime/workflows/refactor/refactor/" \
...
case_duplicate_trigger_fails_validator() {
  ...
  local manifest="$fixture_root/.harmony/orchestration/runtime/workflows/manifest.yml"
  local tmp_file
...
case_capability_map_gap_is_reported() {
  local fixture_root
  fixture_root="$(create_fixture_repo)"
  local capability_map="$fixture_root/.harmony/orchestration/governance/capability-map-v1.yml"
...
case_full_mode_emits_bundle() {
  ...
  [[ -f "$fixture_root/.harmony/output/reports/audits/$run_id/bundle.yml" ]]
  [[ -f "$fixture_root/.harmony/output/reports/audits/$run_id/findings.yml" ]]
  [[ -f "$fixture_root/.harmony/output/reports/audits/$run_id/scores.yml" ]]
  [[ -f "$fixture_root/.harmony/output/reports/audits/$run_id/scenarios.yml" ]]
  [[ -f "$fixture_root/.harmony/cognition/runtime/audits/$run_id/plan.md" ]]
  [[ -f "$fixture_root/.harmony/cognition/runtime/audits/$run_id/evidence.md" ]]
}

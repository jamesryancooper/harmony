...
for row in "${workflow_rows[@]}"; do
...

  {
    ...
    printf '## Purpose\n\n%s\n\n' "$description"
    printf '## Target\n\n'
    printf 'This README summarizes the canonical workflow unit at `.harmony/orchestration/runtime/workflows/%s`.\n\n' "${rel_path%/}"
    printf '## Prerequisites\n\n'
    printf -- '- Required workflow inputs are available.\n'
    printf -- '- Canonical workflow contract exists at `.harmony/orchestration/runtime/workflows/%sworkflow.yml`.\n' "$rel_path"
    ...
    printf '\n## References\n\n'
    printf -- '- Canonical contract: `.harmony/orchestration/runtime/workflows/%sworkflow.yml`\n' "$rel_path"

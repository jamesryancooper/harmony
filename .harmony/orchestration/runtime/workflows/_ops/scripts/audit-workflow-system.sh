...
HARMONY_DIR_DEFAULT="$(cd -- "$ORCHESTRATION_DIR_DEFAULT/../.." && pwd)"
ROOT_DIR_DEFAULT="$(cd -- "$HARMONY_DIR_DEFAULT/.." && pwd)"

MODE="ci-static"
ROOT_DIR="$ROOT_DIR_DEFAULT"
HARMONY_DIR="$HARMONY_DIR_DEFAULT"
...
AUDIT_REPORTS_DIR=""
TMP_DIR=""
SCOPE=".harmony/orchestration/runtime/workflows/"
TARGET=""
FORMAT="markdown"
OUTPUT=""
SEVERITY_THRESHOLD=""
...
init_paths() {
  ...
  ORCHESTRATION_DIR="$HARMONY_DIR/orchestration"
  ...
  RUNTIME_AUDITS_DIR="$HARMONY_DIR/cognition/runtime/audits"
  RUNTIME_INDEX="$RUNTIME_AUDITS_DIR/index.yml"
  REPORTS_DIR="$HARMONY_DIR/output/reports"
...
system_level_checks() {
  ...
  for workflow_id in "${!WORKFLOW_PATHS[@]}"; do
    path="$ROOT_DIR/.harmony/orchestration/runtime/workflows/${WORKFLOW_PATHS[$workflow_id]}"
    if [[ ! -e "$path" ]]; then
...
run_representative_scenarios() {
  ...
  while IFS=$'\t' read -r workflow_id mode expected_format expected_profile; do
    ...
    artifact_path="$ROOT_DIR/.harmony/orchestration/runtime/workflows/$artifact_rel"
...
score_target_workflow() {
  ...
  for id in "${!WORKFLOW_PATHS[@]}"; do
    ...
    if [[ "$(cd "$(dirname "$ROOT_DIR/.harmony/orchestration/runtime/workflows/$artifact_rel")" && pwd)/$(basename "$ROOT_DIR/.harmony/orchestration/runtime/workflows/$artifact_rel")" == "$(cd "$(dirname "$resolved")" && pwd)/$(basename "$resolved")" ]]; then
      workflow_id="$id"
      break
    fi
  done
  if [[ -z "$workflow_id" ]]; then
    ...
    artifact_rel="${target_path#.harmony/orchestration/runtime/workflows/}"

...
description: "Run audit-orchestration in migration-only mode if a migration manifest was provided."
---
...
Run `audit-orchestration` in migration-only mode to check post-migration reference integrity using the same partitioned orchestration path used elsewhere. This step is **skipped** if no migration manifest was provided.
...
1. **Invoke audit-orchestration (migration-only mode):**

   ```text
   /audit-orchestration manifest="{{manifest}}" scope="{{subsystem}}" severity_threshold="{{severity_threshold}}" run_cross_subsystem="false" run_freshness="false" post_remediation="{{post_remediation}}" convergence_k="{{convergence_k}}" seed_list="{{seed_list}}"
   ```
...
**Marker:** `checkpoints/audit-pre-release/02-migration-audit.complete`

## Error Messages

- Workflow failed: "MIGRATION_AUDIT_FAILED: audit-orchestration migration stage exited with errors — {{details}}"
- Manifest invalid: "MANIFEST_INVALID: audit-orchestration rejected the manifest — {{validation_error}}"

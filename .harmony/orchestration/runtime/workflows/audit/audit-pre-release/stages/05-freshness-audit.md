...
# Step 5: Run Freshness And Supersession Audit
...
Run `audit-freshness-and-supersession` to detect stale artifacts, broken supersession chains, and contradictory current-state markers. This step runs by default and can be explicitly skipped with `run_freshness=false`.
...
- Continue to step 6 (merge)
- Note the failure for release recommendation
...
**Marker:** `checkpoints/audit-pre-release/05-freshness-audit.complete`

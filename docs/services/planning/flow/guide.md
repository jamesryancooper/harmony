# Flow — Flows and the Shared LangGraph Runtime

- **Purpose:** Provide a standard, runtime‑agnostic way to configure and run long‑running, multi‑step AI workflows (flows) using explicit graphs wired to Harmony prompts, services, and documents.
- **Responsibilities:** define flow contracts (`FlowConfig`, `FlowRunner`, `FlowRunResult`), call a runner over HTTP (`/flows/run`), model workflow state, orchestrate prompt/action execution, enforce ordering and stop conditions, surface telemetry, and coordinate hand‑offs to other services (Plan, Agent, Prompt, Policy, Observe, etc.).
...
- **Integrates with:** Plan (plans → flows), Spec (specs → flow inputs), Agent (plan‑driven agents that invoke flows), Prompt (prompt assets), Policy/Guard (gates), Observe (traces/metrics/logs), Test/Eval (validation flows), Cursor custom commands (`run` tool) for local developer workflows.
- **I/O:** inputs: canonical prompts (`packages/workflows/**`), workflow manifests (`*.yaml`), Harmony docs (for context); outputs: structured run state, reports (for example, alignment reports), and orchestrated edits proposed for downstream services/agents.
- **Wins:** repeatable, auditable workflows that directly reflect Harmony docs and AI Services Platform services, with clear state and minimal hidden magic.
- **Runtime model:** Flow itself is **runtime‑agnostic**; it assumes "there is some runner that can execute flows" over HTTP. In this monorepo, that runner is the **shared LangGraph runtime** under `agents/runner/runtime/**`, which all flows and agents reuse.
...
Flow sits in **Planning & Orchestration** alongside Spec and Plan. It focuses on the *execution* of multi‑step work once a plan or canonical prompt exists, and delegates actual graph execution to a separate runtime over HTTP.
...
- Calls Harmony prompts and AI Services Platform services as needed.
...
- `SpecToPlanFlow` — chains Spec + Plan + Agent to go from spec → plan → executable run.
...
Flow uses this manifest to build the LangGraph graph wiring.
...
Flow does **not** re‑spec the workflow; it executes what the canonical prompt and manifest encode.
...
Flow is intentionally implemented as a **layered integration** so each concern has a single owner:
...
| `flowkit run <config.flow.json>` | `packages/kits/package.json` bin | Running via installed service binaries (same underlying CLI) |
...
- The Flow CLI is the source of truth for `.flow.json` semantics and validation (`packages/kits/flowkit/src/cli.ts` → `validateFlowConfig`).

#### Locality vs Repo-Wide

- The root `.harmony` is a **repo-wide harness** (see `.harmony/scope.md`), so Flow run procedures belong there.
...
- **Docs:** This guide lives under `docs/services/planning/flow/guide.md`.
...
For full detail, see `docs/services/planning/service-roles.md`. At a glance:

- **Plan**:
  - Owns the **planning** stage between Spec and execution.
  - Consumes Spec artifacts and methodology constraints.
  - Produces BMAD‑style plans and a canonical `plan.json` plus ADR/checklist updates.
- **Agent**:
  - Runs **Plan plans** as durable, stateful agent graphs with retries/resume/HITL.
  - Consumes `plan.json`, decides *which* flows to run and in what order, and maintains durable agent state.
  - Uses Flow + the shared LangGraph runtime as its execution engine; it does **not** own its own runtime.
- **Flow**:
...
1. Spec + Methodology produce/validate a spec.
2. Plan turns spec + constraints into a plan and `plan.json`.
3. Agent reads `plan.json`, decides which flow(s) to run, and constructs Flow `FlowConfig` objects for each step.
4. Flow calls the shared LangGraph runtime (`/flows/run`) for each requested flow.
5. The LangGraph runtime executes the graph and returns structured state/results to Flow (and therefore to Agent or any other caller).

### 2.3 Why LangGraph

Flow uses LangGraph because:
...
- **LLM‑centric:** designed for LLM agents and tools, matching Harmony's AI Services Platform focus.
...
Instead of hard‑coding flows, Flow uses clear layers:
...
Flow's implementation is intentionally minimal and composable: TypeScript contracts and clients live in `packages/kits/flowkit`, and runtime implementations live elsewhere (for example, the shared LangGraph runtime under `agents/runner/runtime/**`).

### 3.1 Directory Layout (example)

In code, Flow is split into:

- **Service contracts and TS interfaces** under `packages/kits/flowkit/` (TypeScript/Node side).
...
On the flow assets side, Flow expects each flow to follow a standardized layout under `packages/workflows/<flowId>/`:
...
2. **Manifest loading**: The `.flow.json` supplies the manifest path; Flow loads it, extracting both the workflow steps and the `assessment` block (docs path, expected files/cross-links, thresholds).
...
- **Metadata**: Includes `run_id` (UUID), `workspace_root` (repo root for provenance), `docs_path`, and manifest-driven expectations for Observe correlation and enforcement.
...
This checks critical Flow assets and verifies that every `packages/workflows/**/*.flow.json` file points at an existing `canonicalPromptPath` and `workflowManifestPath`.
...
Each flow owns a colocated config file that registers it with Flow tooling. Example (`packages/workflows/architecture_assessment/config.flow.json`):
...
Use Studio whenever you need to troubleshoot flow wiring, confirm manifest edits, or demo the orchestration story to other teams. For a conceptual overview of how Studio relates to Flow, Agent, Plan, and the shared runtime, see `service-roles.md`.
...
- `canonicalPromptPath`, `workflowManifestPath`: existing Flow artifacts the runtime consumes.
- `workflowEntrypoint`: node id that Flow should feed into the LangGraph builder.
...
Flow includes a helper for calling the HTTP runner service from Node, so apps and tools (including Cursor commands) can orchestrate flows without re‑implementing the bridge.
...
**Error handling**: The TypeScript service surfaces runner HTTP status codes and response bodies, plus hints about health/availability.

**Result metadata**: `FlowRunResult` includes optional `metadata` with `flowName`, `workflowManifestPath`, `canonicalPromptPath`, `repoRoot`, `runnerEndpoint`, and `runtimeRunId` for Observe correlation.
...
Flow's split between `packages/kits/flowkit` (contracts) and `agents/runner/runtime` (runtime implementation) is deliberate.

### 5.1 Pros (Why it’s the best fit)

- **Architectural clarity**
  - Matches the Harmony Architecture blueprint: services live under `packages/kits` as control‑plane libraries; agentic execution lives under `agents/*`.
  - Keeps Flow's TypeScript contracts separate from any particular runtime or hosting choice.
- **Clean dependency direction**
  - Consumers (apps, agents, Kaizen jobs, CI) depend on Flow via `packages/kits/flowkit`.
  - Flow's TS contracts depend on nothing in `agents/*`; the Python runtime in `agents/runner/runtime` depends on prompts and the workflow manifest, not on app code.
  - Avoids circular patterns like "service imports its own runtime", which would blur control‑plane boundaries.
- **Reuse and composability**
  - Any app (`apps/web`, `apps/api`, `apps/ai-console`, `apps/ai-gateway`) can call Flow via the TS service, regardless of where the runtime is hosted.
  - The runner under `agents/runner/runtime` can host multiple flows (architecture, methodology, comparison, etc.) without changing the Flow contract surface.
- **Methodology alignment**
  - Clean `Spec → Plan → Flow → Implement → Verify → Ship → Learn` chain:
    - Spec validates specs.
    - Plan produces BMAD plans.
    - Flow instantiates flows (via `FlowRunner`), implemented in `agents/runner/runtime`.
    - Agent, Test, Policy, etc. are invoked from inside those flows.
  - Flow is the **flow execution orchestrator**, not an app or a gateway.
- **Future‑ready hosting**
  ...
  - None of those changes require you to move or redesign Flow's TS contracts under `packages/kits/flowkit`.

### 5.2 Cons (and why they’re acceptable)

- **More structure to learn**
  - Contributors must learn:
    - Services → `packages/kits/<service>`.
...
- **Slight indirection**
  - Using Flow from TS involves wiring a `FlowRunner` implementation that calls the Python runtime (CLI or HTTP).
...
Overall, the pros—architectural clarity, reuse, clean layering, and tight alignment with the Methodology—outweigh the added indirection. This layout is the recommended and normative way to implement Flow in this repo.

### 5.3 Responsibility Matrix

Each Flow concern has exactly one owner to prevent drift and duplication:
...
| Concrete flow implementations | `agents/runner/runtime/**` | Should not be imported by TS services |
| Studio wiring | Runtime + repo root | `langgraph.json`, `studio_entry.py` |
| Canonical docs | `docs/services/.../flow/` | This file |
...
- You need to coordinate multiple services (Plan, Agent, Prompt, Policy, Observe, etc.) in a single run.

If a task is:

- Single‑shot, stateless, or trivial → use a direct service call (for example, Agent or Prompt) without a flow.
- Multi‑step, cross‑service, or long‑running → model it as a Flow flow.

Flow's job is to make those flows explicit, reliable, and easy to run from both automation and local tools like Cursor.

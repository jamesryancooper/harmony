...
- **Harmony alignment.** Clean handoff: `SpecKit → PlanKit → FlowKit → AgentKit/TestKit/PolicyKit`, matching the Methodology and AI‑Toolkit READMEs and the canonical kit roles described in `docs/kits/planning-and-orchestration/kit-roles.md`.
...
- **Commands & hooks**: `speckit init …` to author; `speckit validate …` to ensure structure; `speckit render …` to publish. See `docs/kits/README.md` (SpecKit + PlanKit sections) and docs/kits/planning-and-orchestration/speckit/guide.md for wrapper details and contracts.
...
See `docs/kits/README.md` (SpecKit + PlanKit sections) for wrapper details.

---
title: ParseKit — PDF → Markdown (Docling-powered)
description: Developer documentation for implementing and using ParseKit to convert PDFs into clean Markdown with tables and figures, aligned to Harmony’s AI‑Toolkit kit conventions.
---

# ParseKit

Convert PDFs into clean, reviewable Markdown using Docling, with an optional tiny fallback via Microsoft’s MarkItDown. ParseKit is a local‑first kit that produces deterministic artifacts (`.md` files) suitable for downstream ingestion, indexing, and documentation workflows.
...
- Preparing PDFs for doc improvement (Dockit), ingestion (IngestKit), or indexing (IndexKit).
...
- Invariants:
  ...
  - Observability: emit standard run metadata; integrate with ObservaKit where applicable.
  - Governance: use GuardKit redaction policy and typed outputs for CI gates.
...
/kits/parsekit/
  ...
  pyproject.toml
  src/parsekit/cli.py
  schema/parsekit.inputs.v1.json
  schema/parsekit.outputs.v1.json
  runs/            # gitignored artifacts
```

`package.json` (wire a Python kit into Turbo):
...
Normative schema locations (create as the kit matures):
...
- Observability:
  - Emit a parent lifecycle span `kit.parsekit.parse` (when integrated with ObservaKit).
  - Include attributes: `kit.name`, `kit.version`, `run.id`, `stage`.
- Governance:
  - Apply GuardKit redaction rules at write/log boundaries; never serialize secrets/PII.
...
- Store artifacts under `docs_out/parsed` and link run records in PRs (PatchKit).
...
- Slow conversions:
  - Run locally and cache results with CacheKit (pure ops with explicit `cacheKey`).

## Security & Compliance Notes

- Never log or serialize secrets/PII. Apply GuardKit redaction policies.
...
- Dockit (docs improvement), IngestKit (normalize), IndexKit (build stores), QueryKit (answers+evidence).
- ObservaKit for OTel spans/logs; PolicyKit/EvalKit for gates; PatchKit for PRs.

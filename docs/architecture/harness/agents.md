...
- **Perceives** harness state, codebase structure, and task context
...
│  │  refactor, create-harness, synthesize-research, ...    │   │
│  └──────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────┘
```
...
| **Invocation** | Assigned to missions/harnesses | `@mention` or delegation |
...
- **Reading harness state** — `continuity/`, `orchestration/missions/`, `cognition/context/`
...
- Ensure harness-wide consistency
...
.harmony/agency/agents/              # Harness-specific agents
├── README.md                   # Harness agent extensions
...
```yaml
schema_version: "1.0"
default: planner  # Default agent for this harness
...
## Tiered Model (`.harmony/` vs harness-specific)
...
**Merge behavior:** Harness definitions extend (not replace) shared definitions. If a harness agent has the same `name` as a shared agent, the harness definition wins.
...
- [Taxonomy](./taxonomy.md) — Classification of all harness artifacts
- `.harmony/agency/agents/` — Shared agent definitions

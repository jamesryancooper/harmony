# TestKit — Tests & Contracts (Playwright/Pact/Schemathesis)
...
- **Integrates with:** AgentKit (executes checks), DevKit (emits stubs), A11yKit (piggybacks UI flows), SeedKit (fixtures).
...
- **Harmony default:** Treat contracts as hexagonal boundary enforcement; run on every PR as a required gate.

...
- Methodology: kits = concrete tools used to implement the Spec → Plan → Flow → Run lifecycle; see also `docs/kits/planning-and-orchestration/kit-roles.md` for canonical roles.
...
The AI-Toolkit is designed as a cohesive, self-reinforcing system aligned to Harmony’s five pillars. Each pillar is made concrete by specific kits, artifacts, and gates that create a closed-loop from Spec → Plan → Implement → Verify → Ship → Operate → Learn.
...
- **Guided Agentic Autonomy**
 ...
 - **Evolvable Modularity**
  - Hexagonal ports/adapters and kit‑level contracts keep edges (models, providers, tools, runtimes) loosely coupled and hot‑swappable.
  - Modular monolith defaults (Turborepo + StackKit) and stable OpenAPI/JSON‑Schema contracts allow adopting new technologies as plug‑and‑play adapters instead of rewrites.
...
| Evolvable Modularity | StackKit, ToolKit, AgentKit, adapters across kits | Hexagonal ports/adapters and stable contracts make databases, models, providers, and surfaces plug‑and‑play; new capabilities can be added or retired without destabilizing core flows |

...
- `docs/harmony/ai-toolkit/planning-and-orchestration/README.md`
- `docs/harmony/ai-toolkit/planning-and-orchestration/{speckit,plankit,agentkit,flowkit}/guide.md`
- `docs/harmony/architecture/{overview,monorepo-layout,repository-blueprint,agent-roles,knowledge-plane}.md`
...
- In TypeScript (`packages/kits/src/flowkit/**`), it defines:
...
### Step 1 — Define prompts & manifests (FlowKit‑level artifacts)

Under `packages/prompts/**`, add:

- Canonical prompt: describes the flow mission/process.
- Action prompts: one per step.
- YAML workflow manifest: graph structure; node ids, dependencies, prompt paths, meta, etc.
...
- Agent logic is split as follows:
  - The **core agent logic** (state machines, plan execution, hooks, etc.) lives in AgentKit and related kits under `packages/kits` (for example `packages/kits/src/agentkit/**`) as configurations and orchestrators that always call FlowKit.
...
- `FLOWKIT_STUDIO_WORKFLOW_MANIFEST` (for example `packages/prompts/assessment/architecture/workflows/architecture-assessment.yaml`).
...
- **Add new flows** by:
  - Extending prompts/manifests under `packages/prompts/**`.
  - Implementing new LangGraph graphs under `agents/runner/runtime/<flow_name>/`.
  - Wiring handlers in `server.py` and configs in `*.flow.json` and `langgraph.json`.

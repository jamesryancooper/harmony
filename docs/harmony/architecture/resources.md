...
description: Curated external references that complement `docs/harmony/architecture`, with brief annotations and cross-references.
---

# Architecture Suggested Resources

Use this page to curate external references that deepen or complement topics covered in `docs/harmony/architecture`. Keep entries short, annotated, and directly actionable.
...
- Note: Harmony uses Turborepo; this entry is provided for teams using Nx.
...
- Why it matters: Opt‑in streaming of dynamic “holes” while keeping correctness paths deterministic.
- Key takeaways:
...
- Why it matters: GET route handlers are uncached by default; caching must be explicit and justified.
- Key takeaways:

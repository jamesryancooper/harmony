...
Related docs: [overview](./overview.md), [knowledge plane](./knowledge-plane.md), [observability requirements](./observability-requirements.md), [runtime policy](./runtime-policy.md)
...
- Runtime posture: prefer Node runtimes for heavy/long‑running or IO/AI work; reserve Edge for lightweight evaluations (e.g., flags/headers) and read‑mostly endpoints. Keep correctness paths deterministic and server‑side by default.
...
jobs:
  ...
  contracts-drift:
    ...
    steps:
      - uses: actions/checkout@v4
      - run: pnpm -w exec oasdiff ./packages/contracts/openapi.yaml ./dist/openapi.from-code.json --format md > ./kaizen/reports/oasdiff.md || true
...
- Autopilot tasks: docs/links/titles normalization; stale‑flag diffs with owners/expiry; observability scaffolding PRs for missing spans/logs; preview smoke wiring for top routes.
- Copilot tasks (PRs require human approval): contract drift fixes using `oasdiff` on OpenAPI/JSON‑Schema; perf budget nudges with budget delta evidence; targeted threat‑model test stubs (STRIDE‑driven). Contracts and observability jobs above attach artifacts (e.g., `kaizen/reports/oasdiff.md`, trace coverage report) to PRs.

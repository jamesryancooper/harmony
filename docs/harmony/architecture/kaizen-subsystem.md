...
Related docs: [overview](./overview.md), [tooling integration](./tooling-integration.md), [governance model](./governance-model.md), [knowledge plane](./knowledge-plane.md)
...
Complementing the Planner/Builder/Verifier loop, the subsystem uses a simple pipeline that fits the current repo:

- Sensors: CI outcomes, DORA/SRE signals, preview smoke results, OpenTelemetry traces/logs.
...
- Actuators: PR‑opening bots using our prompts/scripts (e.g., docs lint, span scaffolding, contract drift diffs), always behind HITL + branch protection. Weekly summaries are published under `kaizen/reports/`.

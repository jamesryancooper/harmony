...
Related docs: [runtime policy](./runtime-policy.md), [knowledge plane](./knowledge-plane.md), [tooling integration](./tooling-integration.md)
...
We implement **distributed tracing** (using OpenTelemetry or similar) across our modules. This means:
...
- **Propagation**: All module boundaries (especially if in future we have separate processes or threads) must propagate trace context. Use W3C trace context (traceparent/tracestate) with OpenTelemetry. Even within the monolith, propagate context through async/background work.
...
- No PII/PHI in logs/traces: enforce via Guard/Redaction layer (cross‑cutting; see `docs/harmony/architecture/slices-vs-layers.md`) at log/write boundaries; fail checks on violations.

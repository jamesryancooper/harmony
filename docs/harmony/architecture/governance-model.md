...
Related docs: [overview](./overview.md), [runtime policy](./runtime-policy.md), [kaizen subsystem](./kaizen-subsystem.md), [tooling integration](./tooling-integration.md)
...
- Contract Tests Gate: Contract-first interfaces (OpenAPI/JSON Schema) validated via consumer-driven contract tests (e.g., Pact) and fuzz/negative testing (e.g., Schemathesis). Failures block merges/deployments unless explicitly waived.
...
- Autopilot (eligible under Trivial/Low):
  - Docs hygiene PRs (lint/links/titles), stale‑flag cleanup diffs, observability scaffolding (adding missing spans/logs) and preview smoke wiring. These changes still require normal review; no bot approvals or direct pushes are allowed.
- Copilot (Medium/High):
  - Contract drift fixes (OpenAPI/JSON‑Schema with `oasdiff` evidence), performance budget adjustments, threat‑model test PRs. Always require human approvals aligned to the risk rubric and applicable gates.
...
- Feature flags act as a primary runtime control; manual promote/rollback is recorded, and flag state changes are auditable.

Transparency is foundational: approvals, waivers, risk levels, and CI results are recorded and visible for retrospectives and audits.

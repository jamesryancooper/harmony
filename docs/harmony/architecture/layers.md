...
description: Cross‑cutting governance/control‑plane layers in the Harmony architecture, including registry, owners, and required checks — aligned with the Slices vs Layers distinction.
---
...
> Terminology note: In the Harmony architecture, “layer” refers only to cross‑cutting governance/control‑plane concerns (e.g., Improve/Kaizen, quality gates, testing, docs). Runtime code is organized by vertical feature slices with hexagonal (ports/adapters) boundaries. See: [`slices-vs-layers.md`](./slices-vs-layers.md).
...
layers:
  ...
  - name: Improve Layer (Kaizen/Autopilot)
    folders: ["kaizen/", ".github/workflows/kaizen.yaml"]
    codeowners: ["/kaizen/** @team-platform @team-quality"]
...
| -------------------------------- | ------------------------------- | ---------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| Product Surfaces (apps)          | `apps/` · `deploy/`             | `/apps/** @team-apps`, `/deploy/** @team-platform`                                             | build · lint · typecheck · unit · e2e_smoke · container_build  |
| Core Domain & Adapters           | `packages/domain/`              | `/packages/domain/** @team-domain`                                                             | lint · typecheck · unit · contracts_verify                     |
| Contracts & Types                | `packages/contracts/`           | `/packages/contracts/** @team-platform @team-api`                                              | contracts_diff · schemathesis · pact_verify                    |
| Shared UI & Libs                 | `packages/ui/`                  | `/packages/ui/** @team-frontend`                                                               | lint · typecheck · unit · bundle_budget                        |
| Infra & Platform                 | `infra/` · `.github/workflows/` | `/infra/** @team-platform`, `/.github/** @team-platform`                                       | iac_validate · policy_check · ci_selftest                      |
| Quality Gates & Security         | `.github/workflows/` · `infra/ci/` | `/.github/workflows/** @team-platform`, `/infra/ci/** @team-platform`                          | static_analysis · sca_licenses · secret_scan · sbom            |
| Testing                          | `tests/` · `apps/*/tests/`      | `/tests/** @team-quality`, `/apps/*/tests/** @team-quality`                                    | unit · integration · e2e_smoke                                 |
| Observability & SRE              | `infra/otel/` · `apps/*/otel/`  | `/infra/otel/** @team-sre`, `/apps/*/otel/** @team-sre`                                        | otel_coverage · alerts_validate · slo_check                    |
...
| Improve Layer (Kaizen/Autopilot) | `kaizen/` · `.github/workflows/kaizen.yaml` | `/kaizen/** @team-platform @team-quality`                                                      | docs_hygiene · flags_hygiene · otel_scaffold · contracts_drift |
